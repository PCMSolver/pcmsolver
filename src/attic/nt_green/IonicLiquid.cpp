...
Eigen::Array4d IonicLiquid::numericalDirectional(Eigen::Vector3d & sourceNormal_,
        ...
        Eigen::Vector3d & probeNormal_, Eigen::Vector3d & probe_) const
{
    ...
    result(0) = exp(- kappa * (source_ - probe_).norm()) / (epsilon *
                (source_ - probe_).norm());
    ...
    double funcPlus =  exp(- kappa * (source_ - deltaPlus).norm()) / (epsilon *
                       (source_ - deltaPlus).norm());
    double funcMinus = exp(- kappa * (source_ - deltaMinus).norm()) / (epsilon *
                       (source_ - deltaMinus).norm());
    ...
    funcPlus =  exp(- kappa * (deltaPlus - probe_).norm()) / (epsilon *
                (deltaPlus - probe_).norm());
    funcMinus = exp(- kappa * (deltaPlus - probe_).norm()) / (epsilon *
                (deltaMinus - probe_).norm());
...
Eigen::Array4d IonicLiquid::analyticDirectional(Eigen::Vector3d & sourceNormal_,
        Eigen::Vector3d & source_,
        Eigen::Vector3d & probeNormal_, Eigen::Vector3d & probe_) const
{
    Eigen::Array4d result = Eigen::Array4d::Zero();
    ...
    result(1) = (source_ - probe_).dot(probeNormal_) * (1 + kappa * distance ) * exp(
                    - kappa * distance) / (epsilon * distance_3);
    // Directional derivative wrt source_
    result(2) = - (source_ - probe_).dot(sourceNormal_) * (1 + kappa * distance ) * exp(
                    - kappa * distance) / (epsilon * distance_3);
    // Value of the Hessian
    result(3) = sourceNormal_.dot(probeNormal_) * (1 + kappa * distance) * exp(
                    - kappa * distance) / (epsilon * distance_3)
                - pow(kappa, 2.0) * (source_ - probe_).dot(sourceNormal_) * (source_ - probe_).dot(
                    probeNormal_) * exp(- kappa * distance) / (epsilon * distance_3)
                - 3 * (source_ - probe_).dot(sourceNormal_) * (source_ - probe_).dot(
                    probeNormal_) * (1 + kappa * distance) * exp(- kappa * distance) /
                (epsilon * distance_5);

    return result;
}

Eigen::Array4d IonicLiquid::automaticDirectional(Eigen::Vector3d & sourceNormal_,
        Eigen::Vector3d & source_,
        Eigen::Vector3d & probeNormal_, Eigen::Vector3d & probe_) const
{
    Eigen::Array4d result = Eigen::Array4d::Zero();
    ...
    tmp = sqrt((source_(0) - (probe_(0) + probeNormal_(0) * dx)) * (source_(0) - (probe_
               (0) + probeNormal_(0) * dx))
    ...
    // Directional derivative wrt source_
    tmp = sqrt(((source_(0) + sourceNormal_(0) * dx) - probe_(0)) * ((source_
...
Eigen::Array4d IonicLiquid::automaticGradient(Eigen::Vector3d & sourceNormal_,
        Eigen::Vector3d & source_,
        Eigen::Vector3d & probeNormal_, Eigen::Vector3d & probe_) const
{
    Eigen::Array4d result = Eigen::Array4d::Zero();
...
Eigen::Array4d IonicLiquid::automaticHessian(Eigen::Vector3d & sourceNormal_,
        Eigen::Vector3d & source_,
        Eigen::Vector3d & probeNormal_, Eigen::Vector3d & probe_) const
{
    Eigen::Array4d result = Eigen::Array4d::Zero();
