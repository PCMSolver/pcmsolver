...
extern "C" void host_writer(const char * message, PCMSolverIndex message_length);
...
int main(int argc, char * argv[]) {
  ...
  Cavity * cavity = Factory<Cavity, cavityData>::TheFactory().create(
      parsed.cavityType(), parsed.cavityParams());
  ...
  IGreensFunction * gf_i = Factory<IGreensFunction, greenData>::TheFactory().create(
      parsed.greenInsideType(), parsed.insideGreenParams());
  ...
  IGreensFunction * gf_o = Factory<IGreensFunction, greenData>::TheFactory().create(
      parsed.greenOutsideType(), parsed.outsideStaticGreenParams());
  TIMER_OFF("Generating Green's function outside");
  TIMER_ON("Generating solver");
  PCMSolver * solver = Factory<PCMSolver, solverData>::TheFactory().create(
      parsed.solverType(), parsed.solverParams());
  TIMER_OFF("Generating solver");

  BoundaryIntegralOperator * op =
      Factory<BoundaryIntegralOperator, biOperatorData>::TheFactory().create(
          parsed.integratorType(), parsed.integratorParams());
  ...
  out_stream << "Using CODATA " << parsed.CODATAyear() << " set of constants."
             << std::endl;
