...
void UniformDielectric::gradient(Vector3d &gradient, Vector3d &p1, Vector3d &p2, double delta){
    double dist = sqrt((p1 - p2).dot(p1 - p2));
...
double UniformDielectric::evald(Vector3d &direction, Vector3d &p1, Vector3d &p2, double delta){
    double dist = sqrt((p1 - p2).dot(p1 - p2));
    double distcube = dist * dist * dist;
    double der = direction.dot(p2 - p1) / distcube;
    return der;
}
double UniformDielectric::derivative(Vector3d &direction, Vector3d &p1, Vector3d &p2, double delta){
    double dist = sqrt((p1 - p2).dot(p1 - p2));
    double distcube = dist * dist * dist;
