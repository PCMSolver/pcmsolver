...
typedef pcm::unordered_map<std::string, SurfaceFunction> SurfaceFunctionMap;
...
void compute_asc(char * potString, char * chgString, int * irrep)
{
    ...
    SurfaceFunction chg(chgFuncName, _cavity->size());
    chg.vector() = _solver->computeCharge(iter_pot->second.vector(), *irrep);
    chg /= double(_cavity->pointGroup().nrIrrep());
    // Insert it into the map
    if (functions.count(chgFuncName) == 1) { // Key in map already
        functions[chgFuncName] = chg;
    } else { // Create key-value pair
        functions.insert(std::make_pair(chgFuncName, chg));
    }
}
...
void compute_nonequilibrium_asc(char * potString, char * chgString, int * irrep)
{
    ...
    SurfaceFunctionMap::const_iterator iter_pot = functions.find(potFuncName);
    SurfaceFunction chg(chgFuncName, _cavity->size());
    chg.vector() = _noneqSolver->computeCharge(iter_pot->second.vector(), *irrep);
    chg /= double(_cavity->pointGroup().nrIrrep());
    // Insert it into the map
    if (functions.count(chgFuncName) == 1) { // Key in map already
        functions[chgFuncName] = chg;
...
void load_surface_function(const char * name)
{
    ...
    if (dim != _cavity->size()) {
        ...
        if (functions.count(functionName) == 1) { // Key in map already
            functions[functionName] = func;
        } else {
            functions.insert(std::make_pair(functionName, func));
        }
   }
}
...
void set_surface_function(int * nts, double * values, char * name)
{
    ...
    std::string functionName(name);
    if (functions.count(functionName) == 1) { // Key in map already
        functions[functionName].setValues(values);
    } else {
...
void append_surface_function(char * name)
{
    ...
    if (functions.count(functionName) == 1) { // Key in map already
        PCMSOLVER_ERROR("append_surface_function assumes key-value pair not yet in map!");
    } else {
...
{
    return ((sf_map.count(name) == 1) ? true : false);
}
