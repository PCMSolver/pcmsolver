...
pcmsolver_context_t * pcmsolver_new()
{
    return AS_TYPE(pcmsolver_context_t, new pcm::Meddle());
}
...
namespace pcm {
    ...
    {
        SharedIGreensFunction gf_i = Factory<IGreensFunction, greenData>::TheFactory().create(input_->greenInsideType(),
                input_->insideGreenParams());
        SharedIGreensFunction gf_o = Factory<IGreensFunction, greenData>::TheFactory().create(input_->greenOutsideType(),
                input_->outsideStaticGreenParams());
        std::string modelType = input_->solverType();
        K_0_ = Factory<PCMSolver, solverData, SharedIGreensFunction, SharedIGreensFunction>::TheFactory().create(modelType, input_->solverParams(), gf_i, gf_o);
        K_0_->buildSystemMatrix(*cavity_);
        if (input_->isDynamic()) {
            SharedIGreensFunction gf_o_dyn = Factory<IGreensFucntion, greenData>::TheFactory().create(input_->greenOutsideType(),
                    input_->outsideDynamicGreenParams());
            K_d_ = Factory<PCMSolver, solverData, SharedIGreensFunction, SharedIGreensFunction>::TheFactory().create(modelType, input_->solverParams(), gf_i, gf_o_dyn);
