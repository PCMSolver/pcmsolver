    ...
    if (nr_gen > 0) then
      ...
      do i = 1, nr_gen
        if (symop(igen(i))(1:1) == 'C') then
          write(global_print_unit, '(6x, 3a)') 'Rotation about the ', symop(igen(i))(3:3),'-axis'
        else if (symop(igen(i))(1:1) == 'O') then
          write(global_print_unit, '(6x, 3a)') 'Reflection in the ', symop(igen(i))(2:3),'-plane'
        else
...
! 2. Group multiplication table
      ...
      do i = 0, maxrep
        write(global_print_unit,'(4x, a3, 1x, a1, 8(1x, a3, 1x))') symop(jsop(i)), '|', &
                  (symop(ieor(jsop(i), jsop(j))), j = 0, maxrep)
      end do
! 3. Character table
      write(global_print_unit,'(/3x, a/)') '* Character table'
      write(global_print_unit,'(8x, a1, 8(1x, a3, 1x))') '|', (symop(jsop(j)), j = 0, maxrep)
      write(global_print_unit,'(3x, a6, 8a5)') '-----+', ('-----', i = 0, maxrep)
      do i = 0, maxrep
        write(global_print_unit,'(4x, a3, 1x, a1, 8(1x, i3, 1x))') rep(i), '|', (char_tab(jsop(j), i), j=0, maxrep)
      end do
! 4. Direct product table
      write(global_print_unit,'(/3x, a/)') '* Direct product table'
      write(global_print_unit,'(8x, a1, 8(1x, a3, 1x))') '|', (rep(i), i = 0, maxrep)
      write(global_print_unit,'(3x, a6, 8a5)') '-----+', ('-----', i = 0, maxrep)
      do i = 0, maxrep
...
!         number of inversions.
    build_point_group = point_group(group, igroup, nr_gen, maxrep, isymax, jsop, nrots, nrefl, ninvc)
