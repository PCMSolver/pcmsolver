...
void element_element_interaction_pwl(c, P, E, ind1, ind2, RW, Q, R, M, prec, SingleLayer, DoubleLayer, Identity)
...
/* Quadratur mit Genauigkeit prec */
    if (E[ind1].level == E[ind2].level) {       /* nicht unterteilen, da beide Elemente auf gleichem Level */
        ...
        if (dist > eps)
            ...
            CASE = compare_pwl(P, E[ind1].vertex, E[ind2].vertex, &s, &t);      /* gemeinsame Kante/Punkt? */
        ...
        switch (CASE) {
        case 1:
            IntLin1(c, &E[ind1], &E[ind2], &RW[g1], &Q[g1], &Q[g1], R, M, SingleLayer, DoubleLayer);
            break;
        case 2:
            IntLin2(c, &E[ind1], &Q[g1], R, M, SingleLayer, DoubleLayer, Identity);
            break;
        case 3:
            IntLin3(c, &E[ind1], &E[ind2], s, t, &Q[g1], R, M, SingleLayer, DoubleLayer);
            break;
        case 4:
            IntLin4(c, &E[ind1], &E[ind2], s, t, &Q[g1], R, M, SingleLayer, DoubleLayer);
            break;
        default:
...
/* Abspeichern des Integrals */
    if (ind1 >= ind2)
        set_integral_pwl(&E[ind1].interaction, ind2, c);
    else {
        permutate(a, c);
