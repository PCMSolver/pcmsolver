...
unsigned int M;                 /* 2^M*2^M Elemente pro Patch      */
{
    ...
    unsigned int nnz = 0;       /* nunber of nonzero elements of T */
...
/* a-posteriori-Kompression */
    for (i = 0; i < T->n; i++) {
        ...
        for (j = 0; j < T->row_number[i]; j++) {
            if ((fabs(T->value1[i][j]) >= D1[i] * D1[T->index[i][j]] * c[W[i].level][W[T->index[i][j]].level]) || (fabs(T->value2[i][j]) >= D2[i] * D2[T->index[i][j]] * c[W[i].level][W[T->index[i][j]].level])) {
                v1[k] = T->value1[i][j];
                v2[k] = T->value2[i][j];
                w[k] = T->index[i][j];
                k++;
