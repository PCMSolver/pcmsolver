...
{  (*C)[i1] = (unsigned int**) malloc((N+1)*sizeof(unsigned int*));
   ...
   {  (*C)[i1][i2] = (unsigned int*) malloc((N+1)*sizeof(unsigned int));
      for (i3=0; i3<=N; i3++)
      {	 
         if      ((i2 <= N/2) && (i3 <= N/2)) (*C)[i1][i2][i3] = E[ze].vertex[0];
         else if ((i2 <= N/2) && (i3 >  N/2)) (*C)[i1][i2][i3] = E[ze].vertex[1];
         else if ((i2 >  N/2) && (i3 >  N/2)) (*C)[i1][i2][i3] = E[ze].vertex[2];
         else				      (*C)[i1][i2][i3] = E[ze].vertex[3];
...
if ((weight0 == 0) && (weight1 == 0) && (weight2 == 0) && (weight3 == 0)) return;	/* es ist nix zu tun */
...
{  add_element(w1,E,weight*w2->weight[k][0],weight*w2->weight[k][1],\
		weight*w2->weight[k][2],weight*w2->weight[k][3],w2->element[k]);
   }
return;
}
	       ...
	       {  /* 0.+1. Sohn vorhanden -> ueberpruefe auf 2. Sohn */
	          ...
	          if ((s2 < noe) && (fabs(2*W[i].weight[s1][2]-W[i].weight[s1][1]-W[i].weight[s2][2]) < eps))
	             ...
	             {  if ((s3 < noe) && (fabs(2*W[i].weight[s2][3]-W[i].weight[s2][2]-W[i].weight[s3][3]) < eps) \
	                               ...
	                               && (fabs(4*W[i].weight[s0][2]-W[i].weight[s0][0]-W[i].weight[s1][1]-W[i].weight[s2][2]-W[i].weight[s3][3]) < eps))
...
{  free((*W)[i].son);
   ...
   for (j=prototype_number-1; j>=0; j--) if ((*W)[i].weight == (*W)[prototype[j]].weight) break;
...
{  printf("\nWavelet: %d \nLevel:   %d \n%d Soehne:   ",i,W[i].level,W[i].son_number);
   for (j=0; j<W[i].son_number; j++) printf("%d ",W[i].son[j]);
   printf("\n%d Elemente: ",W[i].element_number);
   for (j=0; j<W[i].element_number; j++) 
   {  printf("[%d|(%g,%g,%g,%g)] ",\
   	W[i].element[j],W[i].weight[j][0],W[i].weight[j][1],W[i].weight[j][2],W[i].weight[j][3]);
      }
   printf("\n");
   }
