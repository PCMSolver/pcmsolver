...
RadiiSet initUFF() {
  ...
  //            Atom("Name",    "Symbol", Charge,     Mass,   Radius, Center,
  //            Scaling)
  UFF[0] = Atom("Hydrogen", "H", 1.0, 1.0078250, 1.4430, Origin, 1.1);
  UFF[1] = Atom("Helium", "He", 2.0, 4.0026030, 1.8100, Origin, 1.1);
  UFF[2] = Atom("Lithium", "Li", 3.0, 7.0160050, 1.2255, Origin, 1.1);
  UFF[3] = Atom("Beryllium", "Be", 4.0, 9.0121830, 1.3725, Origin, 1.1);
  UFF[4] = Atom("Boron", "B", 5.0, 11.0093050, 2.0415, Origin, 1.1);
  UFF[5] = Atom("Carbon", "C", 6.0, 12.0000000, 1.9255, Origin, 1.1);
  UFF[6] = Atom("Nitrogen", "N", 7.0, 14.0030740, 1.8300, Origin, 1.1);
  UFF[7] = Atom("Oxygen", "O", 8.0, 15.9949150, 1.7500, Origin, 1.1);
  UFF[8] = Atom("Fluorine", "F", 9.0, 18.9984030, 1.6820, Origin, 1.1);
  UFF[9] = Atom("Neon", "Ne", 10.0, 19.9924390, 1.6215, Origin, 1.1);
  UFF[10] = Atom("Sodium", "Na", 11.0, 22.9897700, 1.4915, Origin, 1.1);
  UFF[11] = Atom("Magnesium", "Mg", 12.0, 23.9850450, 1.5105, Origin, 1.1);
  UFF[12] = Atom("Aluminium", "Al", 13.0, 26.9815410, 2.2495, Origin, 1.1);
  UFF[13] = Atom("Silicon", "Si", 14.0, 27.9769280, 2.1475, Origin, 1.1);
  UFF[14] = Atom("Phosphorus", "P", 15.0, 30.9737630, 2.0735, Origin, 1.1);
  UFF[15] = Atom("Sulphur", "S", 16.0, 31.9720720, 2.0175, Origin, 1.1);
  UFF[16] = Atom("Chlorine", "Cl", 17.0, 34.9688530, 1.9735, Origin, 1.1);
  UFF[17] = Atom("Argon", "Ar", 18.0, 39.9623830, 1.9340, Origin, 1.1);
  UFF[18] = Atom("Potassium", "K", 19.0, 38.9637080, 1.9060, Origin, 1.1);
  UFF[19] = Atom("Calcium", "Ca", 20.0, 39.9625910, 1.6995, Origin, 1.1);
  UFF[20] = Atom("Scandium", "Sc", 21.0, 44.9559140, 1.6475, Origin, 1.1);
  UFF[21] = Atom("Titanium", "Ti", 22.0, 47.9479470, 1.5875, Origin, 1.1);
  UFF[22] = Atom("Vanadium", "V", 23.0, 50.9439630, 1.5720, Origin, 1.1);
  UFF[23] = Atom("Chromium", "Cr", 24.0, 51.9405100, 1.5115, Origin, 1.1);
  UFF[24] = Atom("Manganese", "Mn", 25.0, 54.9380460, 1.4805, Origin, 1.1);
  UFF[25] = Atom("Iron", "Fe", 26.0, 55.9349390, 1.4560, Origin, 1.1);
  UFF[26] = Atom("Cobalt", "Co", 27.0, 58.9331980, 1.4360, Origin, 1.1);
  UFF[27] = Atom("Nickel", "Ni", 28.0, 57.9353470, 1.4170, Origin, 1.1);
  UFF[28] = Atom("Copper", "Cu", 29.0, 62.9295990, 1.7475, Origin, 1.1);
  UFF[29] = Atom("Zinc", "Zn", 30.0, 63.9291450, 1.3815, Origin, 1.1);
  UFF[30] = Atom("Gallium", "Ga", 31.0, 68.9255810, 2.1915, Origin, 1.1);
  UFF[31] = Atom("Germanium", "Ge", 32.0, 73.9211790, 2.1400, Origin, 1.1);
  UFF[32] = Atom("Arsenic", "As", 33.0, 74.9215960, 2.1150, Origin, 1.1);
  UFF[33] = Atom("Selenium", "Se", 34.0, 79.9165210, 2.1025, Origin, 1.1);
  UFF[34] = Atom("Bromine", "Br", 35.0, 78.9183360, 2.0945, Origin, 1.1);
  UFF[35] = Atom("Krypton", "Kr", 36.0, 83.9115060, 2.0705, Origin, 1.1);
  UFF[36] = Atom("Rubidium", "Rb", 37.0, 84.9118000, 2.0570, Origin, 1.1);
  UFF[37] = Atom("Strontium", "Sr", 38.0, 87.9056250, 1.8205, Origin, 1.1);
  UFF[38] = Atom("Yttrium", "Y", 39.0, 88.9058560, 1.6725, Origin, 1.1);
  UFF[39] = Atom("Zirconium", "Zr", 40.0, 89.9047080, 1.5620, Origin, 1.1);
  UFF[40] = Atom("Niobium", "Nb", 41.0, 92.9063780, 1.5825, Origin, 1.1);
  UFF[41] = Atom("Molybdenum", "Mo", 42.0, 97.9054050, 1.5260, Origin, 1.1);
  UFF[42] = Atom("Technetium", "Tc", 43.0, 96.0000000, 1.4990, Origin, 1.1);
  UFF[43] = Atom("Ruthenium", "Ru", 44.0, 101.9043480, 1.4815, Origin, 1.1);
  UFF[44] = Atom("Rhodium", "Rh", 45.0, 102.9055030, 1.4645, Origin, 1.1);
  UFF[45] = Atom("Palladium", "Pd", 46.0, 105.9034750, 1.4495, Origin, 1.1);
  UFF[46] = Atom("Silver", "Ag", 47.0, 106.9050950, 1.5740, Origin, 1.1);
  UFF[47] = Atom("Cadmium", "Cd", 48.0, 113.9033610, 1.4240, Origin, 1.1);
  UFF[48] = Atom("Indium", "In", 49.0, 114.9038750, 2.2315, Origin, 1.1);
  UFF[49] = Atom("Tin", "Sn", 50.0, 119.9021990, 2.1960, Origin, 1.1);
  UFF[50] = Atom("Antimony", "Sb", 51.0, 120.9038240, 2.2100, Origin, 1.1);
  UFF[51] = Atom("Tellurium", "Te", 52.0, 129.9062290, 2.2350, Origin, 1.1);
  UFF[52] = Atom("Iodine", "I", 53.0, 126.9044770, 2.2500, Origin, 1.1);
  UFF[53] = Atom("Xenon", "Xe", 54.0, 131.9041480, 2.2020, Origin, 1.1);
  UFF[54] = Atom("Caesium", "Cs", 55.0, 132.9054290, 2.2585, Origin, 1.1);
  UFF[55] = Atom("Barium", "Ba", 56.0, 137.9052320, 1.8515, Origin, 1.1);
  UFF[56] = Atom("Lanthanum", "La", 57.0, 138.9063470, 1.7610, Origin, 1.1);
  UFF[57] = Atom("Cerium", "Ce", 58.0, 139.9054330, 1.7780, Origin, 1.1);
  UFF[58] = Atom("Praseodymium", "Pr", 59.0, 140.9076470, 1.8030, Origin, 1.1);
  UFF[59] = Atom("Neodymium", "Nd", 60.0, 141.9077190, 1.7875, Origin, 1.1);
  UFF[60] = Atom("Promethium", "Pm", 61.0, 144.9127430, 1.7735, Origin, 1.1);
  UFF[61] = Atom("Samarium", "Sm", 62.0, 151.9197280, 1.7600, Origin, 1.1);
  UFF[62] = Atom("Europium", "Eu", 63.0, 152.9212250, 1.7465, Origin, 1.1);
  UFF[63] = Atom("Gadolinium", "Gd", 64.0, 157.9240190, 1.6840, Origin, 1.1);
  UFF[64] = Atom("Terbium", "Tb", 65.0, 158.9253420, 1.7255, Origin, 1.1);
  UFF[65] = Atom("Dysprosium", "Dy", 66.0, 163.9291710, 1.7140, Origin, 1.1);
  UFF[66] = Atom("Holmium", "Ho", 67.0, 164.9303190, 1.7045, Origin, 1.1);
  UFF[67] = Atom("Erbium", "Er", 68.0, 165.9302900, 1.6955, Origin, 1.1);
  UFF[68] = Atom("Thulium", "Tm", 69.0, 168.9342120, 1.6870, Origin, 1.1);
  UFF[69] = Atom("Ytterbium", "Yb", 70.0, 173.9388590, 1.6775, Origin, 1.1);
  UFF[70] = Atom("Lutetium", "Lu", 71.0, 174.9407700, 1.8200, Origin, 1.1);
  UFF[71] = Atom("Hafnium", "Hf", 72.0, 179.9465457, 1.5705, Origin, 1.1);
  UFF[72] = Atom("Tantalum", "Ta", 73.0, 180.9479920, 1.5850, Origin, 1.1);
  UFF[73] = Atom("Tungsten", "W", 74.0, 183.9509280, 1.5345, Origin, 1.1);
  UFF[74] = Atom("Rhenium", "Re", 75.0, 186.9557440, 1.4770, Origin, 1.1);
  UFF[75] = Atom("Osmium", "Os", 76.0, 191.9614670, 1.5600, Origin, 1.1);
  UFF[76] = Atom("Iridium", "Ir", 77.0, 192.9629170, 1.4200, Origin, 1.1);
  UFF[77] = Atom("Platinum", "Pt", 78.0, 194.9647660, 1.3770, Origin, 1.1);
  UFF[78] = Atom("Gold", "Au", 79.0, 196.9665430, 1.6465, Origin, 1.1);
  UFF[79] = Atom("Mercury", "Hg", 80.0, 201.9706170, 1.3525, Origin, 1.1);
  UFF[80] = Atom("Thallium", "Tl", 81.0, 204.9744010, 2.1735, Origin, 1.1);
  UFF[81] = Atom("Lead", "Pb", 82.0, 207.9766270, 2.1485, Origin, 1.1);
  UFF[82] = Atom("Bismuth", "Bi", 83.0, 208.9803740, 2.1850, Origin, 1.1);
  UFF[83] = Atom("Polonium", "Po", 84.0, 208.9824040, 2.3545, Origin, 1.1);
  UFF[84] = Atom("Astatine", "At", 85.0, 209.9871260, 2.3750, Origin, 1.1);
  UFF[85] = Atom("Radon", "Rn", 86.0, 222.0175710, 2.3825, Origin, 1.1);
  UFF[86] = Atom("Francium", "Fr", 87.0, 223.0197330, 2.4500, Origin, 1.1);
  UFF[87] = Atom("Radium", "Ra", 88.0, 226.0254030, 1.8385, Origin, 1.1);
  UFF[88] = Atom("Actinium", "Ac", 89.0, 227.0277500, 1.7390, Origin, 1.1);
  UFF[89] = Atom("Thorium", "Th", 90.0, 232.0380508, 1.6980, Origin, 1.1);
  UFF[90] = Atom("Protactinium", "Pa", 91.0, 231.0358800, 1.7120, Origin, 1.1);
  UFF[91] = Atom("Uranium", "U", 92.0, 238.0507847, 1.6975, Origin, 1.1);
  UFF[92] = Atom("Neptunium", "Np", 93.0, 237.0000000, 1.7120, Origin, 1.1);
  UFF[93] = Atom("Plutonium", "Pu", 94.0, 244.0000000, 1.7120, Origin, 1.1);
  UFF[94] = Atom("Americium", "Am", 95.0, 243.0000000, 1.6905, Origin, 1.1);
  UFF[95] = Atom("Curium", "Cm", 96.0, 247.0000000, 1.6630, Origin, 1.1);
  UFF[96] = Atom("Berkelium", "Bk", 97.0, 247.0000000, 1.6695, Origin, 1.1);
  UFF[97] = Atom("Californium", "Cf", 98.0, 251.0000000, 1.6565, Origin, 1.1);
  UFF[98] = Atom("Einsteinium", "Es", 99.0, 252.0000000, 1.6495, Origin, 1.1);
  UFF[99] = Atom("Fermium", "Fm", 100.0, 257.0000000, 1.6430, Origin, 1.1);
  UFF[100] = Atom("Mendelevium", "Md", 101.0, 258.0000000, 1.6370, Origin, 1.1);
  UFF[101] = Atom("Nobelium", "No", 102.0, 259.0000000, 1.6240, Origin, 1.1);
  UFF[102] = Atom("Lawrencium", "Lr", 103.0, 266.0000000, 1.6180, Origin, 1.1);
  UFF[103] = Atom("Rutherfordium", "Rf", 104.0, 267.0000000, 0.0000, Origin, 1.1);
  UFF[104] = Atom("Dubnium", "Db", 105.0, 268.0000000, 0.0000, Origin, 1.1);
  UFF[105] = Atom("Seaborgium", "Sg", 106.0, 229.0000000, 0.0000, Origin, 1.1);
  UFF[106] = Atom("Bohrium", "Bh", 107.0, 270.0000000, 0.0000, Origin, 1.1);
  UFF[107] = Atom("Hassium", "Hs", 108.0, 269.0000000, 0.0000, Origin, 1.1);
  UFF[108] = Atom("Meitnerium", "Mt", 109.0, 278.0000000, 0.0000, Origin, 1.1);
  UFF[109] = Atom("Darmstadtium", "Ds", 110.0, 281.0000000, 0.0000, Origin, 1.1);
  UFF[110] = Atom("Roentgenium", "Rg", 111.0, 282.0000000, 0.0000, Origin, 1.1);
  UFF[111] = Atom("Copernicium", "Cn", 112.0, 285.0000000, 0.0000, Origin, 1.1);
  UFF[112] = Atom("Ununtrium", "Uut", 113.0, 286.0000000, 0.0000, Origin, 1.1);
  UFF[113] = Atom("Flerovium", "Fl", 114.0, 289.0000000, 0.0000, Origin, 1.1);
  UFF[114] = Atom("Ununpentium", "Uup", 115.0, 289.0000000, 0.0000, Origin, 1.1);
  UFF[115] = Atom("Livermorium", "Lv", 116.0, 293.0000000, 0.0000, Origin, 1.1);
  UFF[116] = Atom("Ununseptium", "Uus", 117.0, 294.0000000, 0.0000, Origin, 1.1);
  UFF[117] = Atom("Ununoctium", "Uuo", 118.0, 294.0000000, 0.0000, Origin, 1.1);
  // ------------------------------------------------------------
