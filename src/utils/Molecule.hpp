...
public:
   ...
   *
   *  \warning This constructor is to be used **exclusively** when initializing the
   *Molecule
   *  in EXPLICIT mode, i.e. when the user specifies explicitly spheres centers and
   *radii.
   *
   *  Molecule is treated as an aggregate of spheres. We do not have information on
   *the atomic
   *  species involved in the aggregate.
   *  Charges are set to 1.0; masses are set based on the radii; geometry is set from
   *the list of spheres.
