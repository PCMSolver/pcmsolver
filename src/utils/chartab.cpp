...
void CharacterTable::common_init()
{
    ...
    if (!symb.length()) {
        ...
        // exit(1);
        throw std::runtime_error("CharacterTable::CharacterTable: null point group");
    }

    if (make_table() < 0) {
        throw std::runtime_error("CharacterTable::CharacterTable: could not make table");
    }
}
      ...
      bits_(0)
{
    ...
    if (!PointGroup::full_name_to_bits(cpg, bits_)) {
        ...
        throw std::runtime_error("CharacterTable: Invalid point group name provided.");
    }
    common_init();
}
