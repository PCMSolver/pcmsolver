...
int CharacterTable::make_table()
{
    ...
    case PointGroups::D2h:
        ...
        break;
    default:
        throw std::runtime_error("Should not have receached here!");
    }
    ...
    for (gi=0; gi < nirrep_; gi++) {
        ...
        if (gj==nirrep_) {
            ...
            // abort();
            throw std::runtime_error("make_table: uh oh, can't find inverse");
        }
    ...
    if (sym_bits != bits_)
        throw std::runtime_error("make_table: Symmetry operators did not match the point group given.");
