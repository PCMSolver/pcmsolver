...
PointGroup::PointGroup(const std::string& s)
{
    if (full_name_to_bits(s, bits_) == false)
        throw std::runtime_error("PointGroup: Unknown point group name provided.");
...
PointGroup::PointGroup(unsigned char bits, const Eigen::Vector3d & origin)
    : bits_(bits)
{
    set_symbol(bits_to_basic_name(bits));
    origin_ = origin;
...
const char* PointGroup::bits_to_full_name(unsigned char bits)
{
    ...
    default:
        ...
        throw std::runtime_error("Unrecognized point group bits");
    }
}
