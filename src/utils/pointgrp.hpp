...
#ifndef symmetry_pointgrp_h
#define symmetry_pointgrp_h
...
class PointGroup {
  ...
  public:
    ...
    /** Like the above, but this constructor also takes a point of origin
        as an argument. */
    PointGroup(const std::string&,const Eigen::Vector3d&);

    /** Using the bitwise representation constructor the point group object. */
    PointGroup(unsigned char bits);
    /** Using the bitwise representation constructor the point group object. */
    PointGroup(unsigned char bits, const Eigen::Vector3d&);
