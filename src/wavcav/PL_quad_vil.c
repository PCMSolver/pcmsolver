...
void vunc_merg_levm(fajor_sion3D * quad, int p, int q, int ob, int *obs_node, int *n_nd, int *obs_elem, int *n_el)
{
    int as_nd, k;
    as_nd = rech_astr_qonk(*quad, p, q, ob);
    if (quad->elem[p].frvrt == ob)
        quad->elem[p].frvrt = as_nd;
    if (quad->elem[p].scvrt == ob)
        quad->elem[p].scvrt = as_nd;
    if (quad->elem[p].thvrt == ob)
        quad->elem[p].thvrt = as_nd;
    if (quad->elem[p].ftvrt == ob)
        quad->elem[p].ftvrt = as_nd;
    k = *n_nd;
    obs_node[k] = ob;
    *n_nd = k + 1;
    k = *n_el;
...
void durs_disc_vohq(fajor_sion3D * quad, int *obs, int M)
{
    ...
    for (i = 0; i < quad->e_grs; i++) {
        n1 = quad->elem[i].frvrt;
        quad->elem[i].frvrt = map[n1];
        n2 = quad->elem[i].scvrt;
        quad->elem[i].scvrt = map[n2];
        n3 = quad->elem[i].thvrt;
        quad->elem[i].thvrt = map[n3];
        n4 = quad->elem[i].ftvrt;
        quad->elem[i].ftvrt = map[n4];
    }
    for (i = 0; i < k; i++)
...
int seqg_find_gihc_secb(fajor_sion3D * quad, int max_ned, int *force_term)
{
...
void detl_find_faqs_cakq(fajor_sion3D * quad, int max_ned, int *forc_term)
{
    int suc, i, nnd, f_trm;
    *forc_term = 0;
    nnd = quad->n_grs;
...
void tefc_disc_jazq(fajor_sion3D * quad, int z, int nd_a, int nd_b)
{
...
void ciql_remo_cekq(fajor_sion3D * quad, double scale, int max_ned)
{
