...
void gapc_find_bawv_zemg(FILE * fp, megamanif MG, PL_curve * C, int nb_C, point str, point ter)
{
    int n_msh, i, nnd, nel;
...
void muwk_quer_kowd(megamanif MG, PL_curve * C, int nb_C, point str, point ter)
{
    FILE *fp;
...
double qick_diff_gofn(fajor_sion3D QUAD, int z, rgb_lk * col)
{
    int i, e[4], E1, E2, w;
    double res, d;
...
double rohl_diff_lefj(fajor_sion3D QUAD, int z, rgb_lk ref, rgb_lk * col)
{
    int i, e[4], E1, E2, w;
    double res, d;
    e[0] = QUAD.elem[z].frkt;
...
void qihc_aver_hown(fajor_sion3D QUAD, int z, rgb_lk * col, double *rd, double *gr, double *bl)
{
...
void purw_find_ticq_kizq(fajor_sion3D QUAD, int z, rgb_lk * col, double *R, double *G, double *B)
{
    int N = 10, i, q;
    double r, g, b, lrg, d;
    rgb_lk *cand;
...
void dopn_proj_soqv(manif_tl msh, float_curve fc, sphere * SP, int n, ns_curv * B)
{
    int i, j, k, nb, e, E[3];
    double *r;
    point *proj;
...
void fovc_find_tigs_wozp(ns_surf * S, int nb, rgb_lk * col, int task_cur)
{
