...
void temc_deri_cajr(c_curve cc, prop_ccurve * prop)
{
    int i;
    prop->N = cc.N;
    prop->nle = cc.nle;
...
void sart_rect_jamc(double a, double b, double c, double d, c_curve * cc)
{
    int i;
    for (i = 0; i < 4; i++)
        cc->type[i] = 0;
...
void sofl_segm_salc(trmsrf surf, c_curve cc, point * sol)
{
    int nb_ct, i;
    double *T, *PS, t;
    point pre;
