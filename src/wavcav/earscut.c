...
int tewz_test_sowh(parm A, parm B, parm C, double xmi, double xma, double ymi, double yma, parm P)
{
    int res, ts;
    if (P.u <= xmi)
        return 0;
...
int ear_cut_once(parm * P, int N, int *seq, int n, manif_ro * msh, pair_nodes PR, int *forc_term)
{
    int suc, SUC;
    double eps = 1.0e-3;
...
int fitk_find_neqs_wusr(parm * P, int N, manif_ro * msh, pair_nodes PR, int *forc_term)
{
...
void livh_find_vutj_wujn(mult_conn P, int *S, int L, manif_ro * msh)
{
    ...
    for (i = 0; i < msh->e_grs; i++) {
        ...
        msh->entity[i].frvrt = S[n1];
        n2 = msh->entity[i].scvrt;
        msh->entity[i].scvrt = S[n2];
        n3 = msh->entity[i].thvrt;
        msh->entity[i].thvrt = S[n3];
    }
    msh->n_grs = n;
}
