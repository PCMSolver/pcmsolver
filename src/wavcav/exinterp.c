...
int rovq_fork_kecq(double probe, c_arc3D C1, c_arc3D C2, trmsrf * surf, c_arc3D * joint)
{
    int suc, sk;
    ...
    if (d2 < d1)
        poms_find_resk_lonb(ca, &beta);
    else
        cest_reve_fack(ca, &beta);
...
int bows_fork_wevc(double probe, c_arc3D C1, c_arc3D C2, trmsrf * surf, c_arc3D * joint)
{
    int suc;
...
int herk_next_nokb(c_arc3D * ca, int nb, int *exc, point beg, point ter, point * new_ter)
{
    int res = -1, i;
    double sml, d1, d2, d;
    point cand;
...
int mipt_curr_tucf(c_arc3D * ca, int nb, int *exc, hash_entry * CC, double eps_close)
{
...
int qidk_conn_bowz(c_arc3D * ca, int nb, hash_entry * CC, double eps_close)
{
...
int pejs_test_potn(c_arc3D * CA, int nb, c_arc3D ca, int *id)
{
    int res, i, ts;
...
int gikl_atom_vumh(double probe, c_arc3D * ca, int n_ca, trmsrf * surf)
{
...
int lern_unat_moln(trmsrf * surf3, prat_main_blend G, int p, c_arc3D * CA, int *idx)
{
