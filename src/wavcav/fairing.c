...
void mesv_inte_rujm(ns_surf S, int nb, int type, double val_fix, ns_curv * F)
{
    int i;
    double *t, step;
    point *val;
...
void gozn_prop_kupl(ns_surf S, int N, int M, ns_curv1D * phi, ns_curv1D * psi, ns_curv * f, ns_curv * g, point ** x)
{
    int i, j, k, l;
...
void mosn_eval_gonr(double u, double v, int N, int M, ns_curv1D * phi, ns_curv1D * psi, ns_curv * f, ns_curv * g, point ** x, point * sol)
{
    int i, j;
...
void tegj_smoo_gekc(int L_bic, int M_bic, ns_surf S_in, int N_gord, int M_gord, ns_surf * S_out)
{
    int i, j;
    ...
    reck_bicu_bavh(u_bic, v_bic, P, L_bic, M_bic, C_1, C_2, C_3, C_4, S_out);
...
void wemh_norm_mocl(ns_surf S, double u, double v, double h, vect3D * N)
{
    point val, val_ph;
    vect3D U, V;
...
void gurn_norm_tegm(ns_surf S, double u, double v, double h, vect3D * N)
{
...
void cuvk_find_duhr_desl(point ** P, int L, int M, ns_curv A, ns_curv B, ns_curv C, ns_curv D)
{
    int i, j;
...
void hocr_boun_hecr(int L_bic, int M_bic, ns_surf S_in, ns_curv A, ns_curv B, ns_curv C, ns_curv D, ns_surf * S_out)
{
    int i, j;
    double *u_bic, *v_bic, step_u, step_v;
    point **P;
...
void gucd_mixt_vemg(int L_bic, int M_bic, ns_surf S_in_1, ns_surf S_in_2, ns_surf * S_out)
{
    int i, j;
...
void mups_corn_cogj(int side, ns_surf S, point * A, point * B)
{
    int n_u, n_v;
    n_u = S.nu;
    n_v = S.nv;
...
void humz_find_semv_nuwc(vect3D W_cur, vect3D W_ngb, vect3D N_cur, vect3D N_ngb, vect3D * sol)
{
    double c1, c2;
    vect3D N, E1, E2;
...
void wuzv_esti_weql(ns_surf * S, int id_cur, int side_cur, int id_ngb, ns_curv ** T, ns_curv ** N, ns_curv * t)
{
    int side_ngb, i;
...
void legs_side_dolp(ns_surf * S, fajor_sion3D QUAD, int id_cur, int side_cur, ns_curv ** T, ns_curv ** N, ns_curv * t)
{
...
void tofd_find_sofq_fovr(ns_surf * S, fajor_sion3D QUAD, int id_cur, ns_curv ** T, ns_curv ** N)
{
    int L_bic, M_bic;
    prop_n_surf pns;
    prop_n_curv pnc_hrz, pnc_vrt;
    ...
    pnc_hrz.k = S[id_cur].ku;
    pnc_hrz.n = S[id_cur].nu;
    pnc_vrt.k = S[id_cur].kv;
    pnc_vrt.n = S[id_cur].nv;
...
void huvg_find_mucv_newr(fajor_sion3D QUAD, ns_surf * surf, int n_surf)
{
    int i, j;
    ns_curv **T, **N;
    prop_n_curv pnc;
