...
void pesw_stor_neqh(int ex, fajor_sion3D QUAD)
{
    ...
    for (i = 0; i < QUAD.e_grs; i++)
        fprintf(fp, "%d  %d  %d  %d\n", QUAD.elem[i].frvrt, QUAD.elem[i].scvrt, QUAD.elem[i].thvrt, QUAD.elem[i].ftvrt);
    fclose(fp);
}
...
{
    int res;
    if (quad.elem[Q].frvrt == nd)
        res = quad.elem[Q].scvrt;
    if (quad.elem[Q].scvrt == nd)
        res = quad.elem[Q].thvrt;
    if (quad.elem[Q].thvrt == nd)
        res = quad.elem[Q].ftvrt;
    if (quad.elem[Q].ftvrt == nd)
        res = quad.elem[Q].frvrt;
    return res;
}
...
//----
    ...
    if (ind == 1) {
        ...
        //exit(0);
    } else {                    //----(cm[1] is successor of cm[0] within quadrilateral[p])
...
int gudq_find_gecn(fajor_sion3D quad, int el, int *bulk, hash_entry * H)
{
    int id = -1, nel, i, p;
    nel = quad.e_grs;
    for (p = 0; p < H[el].nb; p++) {
...
int wuzv_expa_cogm(fajor_sion3D * quad, int *bulk, hash_entry * H)
{
