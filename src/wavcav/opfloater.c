...
void rotw_find_wuhq(manif_tl M, parm * D, int N, int *bd, int nbd, int *corner, parm * img)
{
    int i, j, k, m, p, q, f, g;
    double *l, L, lambda;
    parm s, e;
...
void pitd_neig_weqj(manif_tl msh, int **neighb, int *nb_neighb, int **incident_ed, int maxinc)
{
...
void nulc_find_newl(manif_tl msh, int i, int *nb_neighb, int **incident_ed, int *trg)
{
...
double gofr_lamb_furp(manif_tl msh, int i, int j, int **neighb, int *nb_neighb, int **incident_ed)
{
...
void hoqt_asse_guwn(manif_tl msh, int *mu, int nin, int **neighb, int *nb_neighb, int **incident_ed, double **A)
{
    int i, j, k, r, s, ts, n;
    double sum, temp;
    for (i = 0; i < nin; i++)
...
void dejm_righ_tupc(manif_tl msh, int *mu, int nin, int *bd, int nbd, double *y, int **neighb, int *nb_neighb, int **incident_ed, double *rhs)
{
    int i, r, s, k, n, ts, l;
    double entry, temp;
    for (i = 0; i < nin; i++) {
...
void zecp_find_bogj(manif_tl msh, int *mu, int nin, int *bd, int nbd, int **neighb, int *nb_neighb, int **incident_ed, parm * y, parm * x)
{
    int i;
...
void fewm_floa_cifj(manif_tl msh, parm * D, int N, int *bd, int nbd, int *corner, manif_ro * mshout)
{
    ...
    zecp_find_bogj(msh, mu, nin, bd, nbd, neighb, nb_neighb, incident_ed, img, internal);
    for (i = 0; i < nnd; i++)
        free(neighb[i]);
    free(neighb);
    for (i = 0; i < nnd; i++)
