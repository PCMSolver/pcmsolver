...
void gopr_coll_sivw(manif_tl * msh, int f, int *obs_nd, int *nb_ob_nd, int *obs_el, int *nb_ob_el, point P)
{
    ...
    *nb_ob_nd = k + 1;
    ...
    for (i = 0; i < nb; i++) {
        e = inc[i];
        if (msh->entity[e].frvrt == n2)
            msh->entity[e].frvrt = n1;
        if (msh->entity[e].scvrt == n2)
            msh->entity[e].scvrt = n1;
        if (msh->entity[e].thvrt == n2)
            msh->entity[e].thvrt = n1;
...
void wacl_coll_hijt(manif_tl * msh, int *ed_col, int nb_col, int *map_nd, int *map_el)
{
    ...
    for (i = 0; i < msh->e_grs; i++) {
        ...
        if (ts == 0) {
            ...
            temp_elem[k].frvrt = map_nd[m1];
            m2 = msh->entity[i].scvrt;
            temp_elem[k].scvrt = map_nd[m2];
            m3 = msh->entity[i].thvrt;
            temp_elem[k].thvrt = map_nd[m3];
            map_el[i] = k;
            k++;
        } else
            map_el[i] = -1;
...
int test_periodic_topo(manif_tl msh, int z1, int z2, int n1, int n2, int *inc, int nb)
{
...
double analyse_anis(manif_tl nar)
{
    ...
    homs_boun_gosm(nar.knot, nar.n_grs, &xmi, &xma, &ymi, &yma, &zmi, &zma);
...
int tunc_next_bihv(double scl1, double scl2, manif_tl nar, int beg, int *exc, int n_exc, int *E)
{
...
int fahg_edge_lujd(double scl, double scl2, manif_tl msh, int *ed_col, int max, int *inv_node, int *n_inv, int max_inv)
{
