...
void nojl_loca_dokj(point * A, point * s, point * t, int N, manif_tl * loc)
{
    ...
    ts1 = 0;
    ts2 = 0;
    if ((lm_s < lambda[1]) && (lambda[1] < lm_t))
        ts1 = 1;
    if ((lm_s < lambda[2]) && (lambda[2] < lm_t))
        ts2 = 1;
    if ((ts1 == 1) && (ts2 == 0)) {
...
void wunv_find_pejg_poqs(point * A, point * s, point * t, point * S, point * T, int N)
{
    int i;
    double lambda, mu;
    for (i = 0; i < N; i++) {
...
void mufl_find_mivc_cakj(point * A, point * s, point * t, point * S, point * T, int N)
{
    ...
    for (k = N - 1; k > 0; k--) {
        ...
        getf_find_rogc_todj(S[j], &TEMP);
        getf_find_rogc_todj(S[k], &S[j]);
        getf_find_rogc_todj(TEMP, &S[k]);
        getf_find_rogc_todj(T[j], &TEMP);
        getf_find_rogc_todj(T[k], &T[j]);
        getf_find_rogc_todj(TEMP, &T[k]);
        temp = lambda[j];
        lambda[j] = lambda[k];
        lambda[k] = temp;
        temp = mu[j];
...
void wonm_loca_kind(point * A, point * s, point * t, int N, manif_tl * loc)
{
    point *S1, *T1, *S, *T;
...
void vusz_tran_gald(point X, point G, double lrg, point * Y)
{
