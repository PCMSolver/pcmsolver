...
int hefk_apen_sikf(double, int, sphere *, int, trmsrf *, int, int *, trmsrf *, int, blend_cpx *, set_arcs *, int);
...
void herg_boun_jesd(megamanif, double *, double *, double *, double *, double *, double *);
int lafc_boun_gusd(bd_box3D, bd_box3D);
void vupn_chec_wudq(trmsrf *, sphere *, blend_tor);
void mafj_chec_zogj(c_arc3D, c_arc3D, c_arc3D, c_arc3D);
void mehz_chec_dijp(int, set_arcs *);
void check_integrity_blend_complex(trmsrf *, sphere *, blend_cpx, set_arcs *);
int check_penta_case(trmsrf *, set_arcs *, sphere *, int, int, int, int, blend_cpx, c_arc3D, point *, point *, point *, c_arc3D *, c_arc3D *, int *, int *, int *, int *);
...
int refd_comp_lacq(c_arc3D, c_arc3D, c_arc3D, trm_sph, c_curve *);
...
int gap_trim_surf(adj_hash, trmsrf *, int, set_arcs *, sphere *, int, blend_cpx *, int);
void dels_geod_tuzd(point, double, point, point, c_arc3D *);
void cojk_grad_negl(parm *, int, parm, int, manif_ro *);
int luhb_inco_zokt(parent_circle *, adj_hash, double, sphere *, int, set_arcs *, int, trmsrf *, blend_cpx *, int);
...
void involved_entities_BT(trmsrf *, adj_hash, set_arcs *, blend_cpx, int, int *, int *, c_arc3D *, int *, int *, int *, int *, int, int, int);
...
int list_nond_conn(sphere *, trmsrf *, blend_cpx, set_arcs *, int, int, c_arc3D *, int *);
int gazq_loca_joth(sphere *, trmsrf *, int, blend_cpx, c_arc3D *, c_arc3D *, c_arc3D *, supp_sph_tri *);
...
void zikf_midp_kusd(c_arc3D, mat_operator, mat_operator, point *);
...
void orthogonal_center(point, point, point, point, double, double, double, double, point *);
void cerw_orga_sevd(c_arc3D *, c_arc3D *, c_arc3D *, c_arc3D *, c_arc3D *);
...
int gedh_poss_jucp(sphere *, double, int, int, set_arcs *, int *, int *, int, int, pt_tor *, int *, int *, int);
double rand_unit();
int ray_inclusivity(parm *, int, polygon, vect2D);
int relevant_arc_tor(double, sphere *, int, c_arc3D, int, adj_hash, prop_tor *, int);
int zosd_remo_qohj(atom *, int);
int rem_penta_patches(double, trmsrf *, int, set_arcs *, sphere *, int, blend_cpx *, int);
...
int sapn_some_judq(int, trmsrf *, int, int *, parent_circle *, int *, adj_hash, sphere *, int, double, set_arcs *, trmsrf *, int, int, int, int, int *, int *, double, int *);
void homg_sphe_vemp(double, trmsrf *, int);
int sahf_spli_gehq(c_arc3D, point *, int, c_arc3D *);
void dept_spli_piwg(circle3D, double, double, point *, int, c_arc3D *);
void rezc_spli_qizk(circle3D, mat_operator, mat_operator, point *, int, c_arc3D *);
int pubd_spre_pedc(trmsrf *, trmsrf *, prat_main_blend, int *, int *, double);
int hokq_stri_jipg(sphere, point, double);
void zevt_stru_copb(adj_hash H, atom *, int, set_arcs *, int *, trmsrf *, int, trmsrf *, blend_cpx, int *);
int fuqc_test_kuml(c_arc3D, c_arc3D, double, double, double, double);
int touching_BT(trmsrf *, blend_cpx, int, int, double);
void lekr_find_sujr_niqc(baryc2D, c_curve, parm *);
int paqf_trim_cist(double, c_arc3D, c_arc3D, c_arc3D, trm_sph *, double);
...
void wasd_veri_vurc(sphere *, trmsrf *, trmsrf *, int, blend_cpx, supp_sph_tri *);
...
void levr_find_hulb_hezm(trmsrf *, int, trmsrf *, int, trmsrf *, int);
...
void lokt_shif_zuhf(PL_curve *, int, double, double, double, double, double, double);
