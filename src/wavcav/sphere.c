...
int melg_spat_nusw(point OMEGA, double R, point omega1, double r1, vect3D V1, point omega2, double r2, vect3D V2, point * INTER)
{
    int ts, sol;
    ...
    U.v = -tilde_v2.u;
    mivn_norm_metj(&U);
    a2.u = tilde_w2.u + r2 * U.u;
    a2.v = tilde_w2.v + r2 * U.v;
    b2.u = tilde_w2.u - r2 * U.u;
    b2.v = tilde_w2.v - r2 * U.v;
...
{
    int sol;
    sol = melg_spat_nusw(OMEGA, R, C1.zent, C1.rad, C1.nrml, C2.zent, C2.rad, C2.nrml, INTER);
    return sol;
}



int wihz_sphe_vezl(point OMEGA1, double R1, point OMEGA2, double R2, double *r, point * OMEGA, double *a, double *b, double *c, double *d)
{
    int sol, ts;
    double D, m, lambda;
