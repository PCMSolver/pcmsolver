...
void hetc_glob_kaqc(double probe, double c_param, sphere * S, int nb_sph, adj_hash H, int *forc_term)
{
    ...
    *forc_term = 0;
    ...
    nb_surf2 = sapn_some_judq(meth, surf1, nb_surf1, supp, PC, &nb_pc, H, S, nb_sph, probe, SA, surf2, 0, max_tor, max_self, MAX_TRIM_SURF2, V1, V2, thres_stretch, &f_trm);
    if (f_trm == 1) {
    ...
    if (f_trm == 0) {
        ...
        new_surf = luhb_inco_zokt(PC, H, probe, S, nb_sph, SA, nb_surf2, surf2, &BC, MAX_TRIM_SURF2);
        raqn_inve_kotj(nb_sph, SA);
    }
    free(PC);
    if (f_trm == 0) {
    ...
    if (f_trm == 0) {
        ...
        new_surf = hefk_apen_sikf(probe, 6, S, nb_sph, surf1, nb_surf1, supp, surf2, nb_surf2, &BC, SA, MAX_TRIM_SURF2);
        nb_surf2 = new_surf;
        ...
        if (f_trm3 == 0) {
            ...
            new_surf = qumw_atom_picm(S, probe, surf1, nb_surf1, surf2, nb_surf2, surf3, nb_surf3, SA, BC, 1.0e-4, MAX_TRIM_SURF3, gap, &n_gap, max_gap, max_gap_val);
            nb_surf3 = new_surf;
        }
        ...
        if (f_trm3 == 0) {
            ...
            luvm_blen_foms(S, nb_sph, supp, surf1, nb_surf1, surf2, nb_surf2, surf3, nb_surf3, SA, BC, gap, n_gap, c_param, &f_trm2);
            if (f_trm2 == 1) {
                      ...
                      const char* infile)
{
    ...
    printf("In waveletCavityDrv_\n");
    tmpout = fopen("create_cavity.out", "w");
    printf("tmpout created\n");
    fprintf(tmpout, "Probe Radius %20.12f Coarsity %20.12f Patch Level %3d\n", probeRadius, coarsity, patchLevel);
    printf("printed some info to tmpout\n");
    test = bihp_test_romg(infile, probeRadius, coarsity, &dummy);
    printf("run a test...\n");
    fclose(tmpout);
    printf("tmpout closed\n");
