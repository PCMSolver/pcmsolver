...
int mefj_list_hepq(parent_circle * PC, adj_hash H, int w1, int w2, sphere * S, int nb_sph, double probe, set_arcs * SA, int *ar1, int *ar2, int *nb1, int *nb2)
{
...
void lotb_idea_hinq(point omega1, point omega2, point A, circle3D c3D, point * SIB)
{
    ...
    for (i = 0; i < 2; i++) {
        if (i == 0)
            dis0 = wodt_dist_gilq(A, J[i]);
        if (i == 1)
            dis1 = wodt_dist_gilq(A, J[i]);
    }
    if (dis0 < dis1)
        getf_find_rogc_todj(J[0], SIB);
    else
        getf_find_rogc_todj(J[1], SIB);
    free(J);
}


void laqv_inte_jotl(c_arc2D C, double *a, double *b)
{
...
int biqf_blen_lapc(double probe_init, point omega1, point omega2, point P1, point P2, c_arc3D * c)
{
    ...
    if (suc == SUCCESS) {
        ...
        for (p = 0; p <= nb_trials; p++) {
            ...
            if (q == 0)
                filq_geod_howz(cl1, A[0], A[1], &ca2D);
            if (q == 1)
                filq_geod_howz(cl2, A[0], A[1], &ca2D);
...
int tegc_toro_sekf(point omega1, point omega2, double probe, c_arc3D C1, c_arc3D C2, pt_tor * PT)
{
    int suc1, suc2, suc;
    c_arc3D *CA;
...
int fosh_toro_zobl(point omega1, point omega2, double probe, c_arc3D C1, c_arc3D C2, pt_tor * PT)
{
    int suc;
    double d1, d2;
    c_arc3D C2_tilde;
...
double zofp_erro_pizh(point omega1, point omega2, c_arc3D c_a, c_arc3D c_b)
{
    int i, j, q;
...
double vokz_erro_sufq(set_arcs * SA, int w1, int w2, int a1, int a2, sphere * S)
{
    double err;
    point omega1, omega2;
...
int gedh_poss_jucp(sphere * S, double probe, int w1, int w2, set_arcs * SA, int *ar1, int *ar2, int nb1, int nb2, pt_tor * PT, int *arc_id1, int *arc_id2, int max_pos)
{
    ...
    for (i = 0; i < nb1; i++) {
        ...
        if ((q_a1 != -1) && (q_a2 != -1)) {
            ...
            suc = fosh_toro_zobl(omega1, omega2, probe, SA[w1].C[q_a1], SA[w2].C[q_a2], &PT[nb]);
            if (suc == SUCCESS) {
                arc_id1[nb] = q_a1;
                arc_id2[nb] = q_a2;
                exc[q_j] = 1;
