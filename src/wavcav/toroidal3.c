...
int fiws_inte_puds(parent_circle * PC, adj_hash H, sphere * S, int nb_sph, double probe, int w1, int w2, set_arcs * SA, pt_tor * PT, int *arc_id1, int *arc_id2, int *map1, int *map2, int max_pos)
{
    ...
    n_loc = gedh_poss_jucp(S, probe, w1, w2, SA, ar1, ar2, nb1, nb2, PT, arc_id1, arc_id2, max_pos);
...
void rakg_veri_cizm(sphere * S, pt_tor * PT, int nb, set_arcs * SA, int w1, int w2, double eps)
{
    int j;
    double d1, d2;
    for (j = 0; j < nb; j++) {
...
int luhb_inco_zokt(parent_circle * PC, adj_hash H, double probe, sphere * S, int nb_sph, set_arcs * SA, int nb_cur, trmsrf * surf, blend_cpx * BC, int max_surf)
{
    ...
    for (w1 = 0; w1 < nb_sph; w1++) {
        ...
        for (w2 = 0; w2 < w1; w2++) {
            ...
            if (ts == 0) {
                ...
                nb = fiws_inte_puds(PC, H, S, nb_sph, probe, w1, w2, SA, PT, arc_id1, arc_id2, map1, map2, m);
                if (nb >= m) {
