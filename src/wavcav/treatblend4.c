...
void decomp_one_conv_aux(atom * S, int nb_sph, blend_cpx BC, set_arcs * SA,
                         trmsrf * surf1, int q, trmsrf * surf2, int *supp, int *n_len, megamanif * MG, prat_main_m * GM, int mx_edge_m, double err, int depth, supp_surf * sp_sr, msh_corn * M_C, point ** MID, arc_supp * A, int *forc_term)
{
    ...
    *forc_term = 0;
    ...
    muhs_disc_kesq(S, surf1[q], surf2, BC, z, SA[z], n_len, dis_ex, dis_in, endp, A[z]);
    ned_m = GM->k_grs;
    for (j = 0; j < nb_loc; j++) {
        r = endp[j];
    ...
    suc = wehc_sphe_bufp(surf1[q], S[supp[q]], P, &msh, err, depth, nnd2D, nel2D, ned2D, &f_trm);
    if (f_trm == 1) {
        *forc_term = 1;
...
void maillage_blend(int nnd, double ideal_length, trmsrf * surf2, int *discr_param, prat_main_m * GM, prat_main_blend G, int *n_len, included_sides * inc, megamanif * MG, supp_surf * sp_sr, msh_corn * M_C, point ** MID, int *forc_term)
{
...
void rukg_tria_sufl(int nnd, double ideal_length, trmsrf * surf2, int *discr_param, prat_main_m * GM, prat_main_blend G, int *n_len, included_sides * inc, megamanif * MG, supp_surf * sp_sr, msh_corn * M_C, point ** MID)
{
    int i, j, k, *crn, e, n_width_bl, w, E;
    int side_1, side_2, nnd2D, nel2D, ned2D;
    int nnd_pl, nel_pl, ned_pl, max_leg = 20;
...
int mult_conn_surf2(trmsrf surf, int *dis_ex, int **dis_in, mult_conn * P, int *corner, msh_corn * M_C, point * mid)
{
...
void disc_parm_trimmed2(sphere * S, trmsrf ts, trmsrf * surf2, blend_cpx BC, int z, set_arcs SA, int *n_len, int *dis_ex, int **dis_in, int *endp)
{
...
void spherical_triangulate_mult_conn2(trmsrf surf, sphere S, mult_conn P, manif_ro * msh, double accuracy, int max, int mx_nnd, int mx_nel, int mx_ned)
{
...
void non_spherical_triangulate_mult_conn2(trmsrf surf, mult_conn P, manif_ro * msh, double accuracy, int max, int mx_nnd, int mx_nel, int mx_ned)
{
    int suc, level = 4, f_dummy;
    sphere dummy;
    manif_ro init;
...
void jukg_deco_lehf(atom * S, blend_cpx BC, set_arcs * SA, trmsrf * surf1, int q, trmsrf * surf2, int *supp, int *n_len, megamanif * MG, prat_main_m * GM, int mx_edge_m, double err, int depth, supp_surf * sp_sr, msh_corn * M_C, point ** MID)
{
    int j, nnd2D, nel2D, ned2D, z, nc, nvt, *crn, nb_cr, nb_loc;
    ...
    ts_spec = fizt_chec_fuwk(P, 1.0e-3, &omega);
    spherical_triangulate_mult_conn2(surf1[q], S[supp[q]], P, &msh, err, depth, nnd2D, nel2D, ned2D);
...
void huvr_deco_gajl(trmsrf * surf3, int *discr_param, prat_main_blend G, int q, megamanif * MG, prat_main_m * GM, double err, int depth, supp_surf * sp_sr, msh_corn * M_C, point ** MID)
{
...
void jezc_find_wuvb_mung(int type, atom * S, int nb_sph, int *supp,
                         ...
                         included_sides * inc, prat_main_blend G, prat_main_m * GM, int mx_edge_m, supp_surf * sp_sr, msh_corn * M_C, point ** MID, double ideal_length, double err, int depth, double err_surf3, megamanif * MG, int *forc_term)
{
    ...
    rukg_tria_sufl(nnd, ideal_length, surf2, discr_param, GM, G, n_len, inc, MG, sp_sr, M_C, MID);

    for (i = 0; i < nnd; i++)
        if (G.N[i].type_node == T_TYPE) {
            fprintf(tmpout, "trimmed surf[%d/%d]  of type(T) \n", i, nnd - 1);
            huvr_deco_gajl(surf3, discr_param, G, i, MG, GM, err_surf3, depth, sp_sr, M_C, MID);
        }

    fprintf(tmpout, "treating surf1\n");
    for (i = 0; i < nb_surf1; i++) {
        fprintf(tmpout, "i=%d/%d    ", i, nb_surf1 - 1);
        jukg_deco_lehf(S, BC, SA, surf1, i, surf2, supp, n_len, MG, GM, mx_edge_m, err, depth, sp_sr, M_C, MID);
    }
    if (type == 0) {
        prz = 0.0;
        p = MG->mw_grs;
...
void sazk_disc_tujc(prop_discr pd_c, prop_discr pd_f,
                    ...
                    supp_surf * sp_sr_c, supp_surf * sp_sr_f, prat_main_m * GM_c, prat_main_m * GM_f, int mx_edge_m, msh_corn * M_C_c, msh_corn * M_C_f, point ** MID_c, point ** MID_f, int *force_term)
{
    ...
    *force_term = 0;
    ...
    G.k_grs = 0;
    cetm_find_cojq_vurg(surf2, surf3, B, &G, exc, inc, eps_min, eps_max, n_trials);
    free(exc);
    free(B);
    free(map);
    ...
    jezc_find_wuvb_mung(0, S, nb_sph, supp, SA, BC, surf1, nb_surf1, surf2, nb_surf2, surf3, nb_surf3, inc, G, GM_c, mx_edge_m, sp_sr_c, M_C_c, MID_c, ideal_length_c, err_c, depth_c, c_err_surf3, MG_c, &f_trm);
    if (f_trm == 1) {
        *force_term = 1;
        free(G.N);
        free(G.E);
    ...
    jezc_find_wuvb_mung(1, S, nb_sph, supp, SA, BC, surf1, nb_surf1, surf2, nb_surf2, surf3, nb_surf3, inc, G, GM_f, mx_edge_m, sp_sr_f, M_C_f, MID_f, ideal_length_f, err_f, depth_f, f_err_surf3, MG_f, &f_trm);
    if (f_trm == 1) {
        fprintf(tmpout, "force term: jezc_find_wuvb_mung() in sazk_disc_tujc()\n");
        *force_term = 1;
        free(G.N);
