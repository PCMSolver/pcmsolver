...
void sung_trim_gasq(sphere Si, sphere Sj, circle3D C, trm_sph * Ti, trm_sph * Tj)
{
    int ts;
    double rj;
    point A;
...
void sonv_trim_gopl(sphere * S, int n, trm_sph * T, adj_hash H)
{
    int i, j, val, ts, q, s;
    double r, a, b, c, d, lg;
    circle3D C, *temp;
    ...
    for (q = 0; q < n; q++) {
        ...
        for (j = 0; j < val; j++) {
            ...
            ts = wihz_sphe_vezl(S[q].zent, S[q].rad, S[i].zent, S[i].rad, &r, &omega, &a, &b, &c, &d);
            if (ts == 2) {
...
void rijn_rear_woqp(ns_curv * C, int n, int *idx, double eps, int m)
{
...
int rofq_extr_hopm(ns_curv * C, int n, int *excl, int p, double eps, int *idx)
{
...
void fuch_find_tams(ns_curv * C, int n, double eps, trm_ids * I)
{
...
void wiqs_conn_vomw(ns_curv * C, trm_ids I, c_curve * cc)
{
    int i, p, N, s;
...
int wong_comp_golp(trm_sph T, c_arc3D * C_loc, int n, c_curve * cc)
{
    int m, i, n_loc;
    double eps = 0.01;
    prop_n_curv pnc;
...
void pugj_crea_sotm(int r, int *list, int m, trm_sph T, c_curve * cc, int n, trmsrf * S, int *orient)
{
    int ort, i, k;
    S->nb_inner = n - 1;
...
int hect_list_mudc(sphere * S, int nb_sph, adj_hash H, set_arcs * SA, trmsrf * surf, int *supp, int max_surf)
{
    ...
    for (i = 0; i < nb_sph; i++)
        if (SA[i].ar_grs != 1) {
            ...
            p = qonk_form_sevt(i, T[i], nb_arcs, cc, nb_comp, surf, nb_cur, supp, max_surf);
            nb_cur = nb_cur + p;
            for (j = 0; j < nb_arcs; j++)
