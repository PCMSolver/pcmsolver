...
void arc_disc_param(blend_cpx BC, trmsrf * surf2, int z, set_arcs SA, arc_supp * A)
{
...
int coarse_ext_edges(parm * A, parm * B, int nb_in, mult_conn mc, add_mc AM, int *res)
{
    ...
    for (i = 0; i < ned; i++)
        if (AM.EMC[i].pos == -1) {
            ...
            nx = AM.EMC[i].n_ter;
            for (j = 0; j < nb_in; j++) {
                ts = kesn_segm_lafn(A[j], B[j], mc.vertex[pr], mc.vertex[nx], marg, eps__, mu);
                if (ts == 1) {
                    res[k] = i;
                    k++;
                    break;
...
int id_blend_for_coarse(trmsrf ts, supporting S, mult_conn mc, edge_mc e, set_arcs SA, arc_supp A)
{
