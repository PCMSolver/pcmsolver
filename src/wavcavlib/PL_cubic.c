...
int nerv_find_wovt_nulg(double T,point *sam,int n_sam,double *len,
...
void vefm_eval_bilt(double t,point *sam,int n_sam,
double *len,point *P)
{int i,q;
...
void talv_quad_bapt(ns_surf *surf,
int n_surf,int N,int M,fajor_sion3D *QUAD)
...
for(j=0;j<MG.mw_grs;j++)
	{nnd=MG.msh[j].n_grs;
	for(i=0;i<nnd;i++)
		fprintf(fp,"%f  %f  %f\n",MG.msh[j].knot[i].absi,
		MG.msh[j].knot[i].ordo,MG.msh[j].knot[i].cote);
	}
fclose(fp);
}
...
void lesm_find_vung(ns_surf *S,int n_surf,
int N,int nb_fin,PL_curve *PL)
{int i,j,k;
PL_curve *pl;
