...
void pifc_quad_hufr(point S0,point S1,
point S2,bez_crv *X)
...
void kedz_find_tuvq_qovc(bez_crv al,bez_crv bt,
bez_crv gm,bez_crv dt,double u,double v,point *sol)
...
void wetc_part_fevw(int var,parm Q,bez_crv al,bez_crv bt,
bez_crv gm,bez_crv dt,vect3D *U)
{double u,v,h=1.0e-3;
point Rph,R;
u=Q.u;
v=Q.v;
...
Q[0].u=marg;		Q[0].v=marg;
Q[1].u=1.0-marg;	Q[1].v=marg;
Q[2].u=1.0-marg;	Q[2].v=1.0-marg;
Q[3].u=marg;		Q[3].v=1.0-marg;
res=+LARGE_NUMBER;
...
if(type==1)
	...
	lrg=-LARGE_NUMBER;
	for(i=0;i<4;i++)
		{if(len[i]>lrg)	lrg=len[i];
		if(len[i]<sml)	sml=len[i];
		}
	free(len);
	scl=sml/lrg;
	res=scl*res;
