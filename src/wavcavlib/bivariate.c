...
void copm_para_kesl(double *u,int L,double *v,int M,
...
void vubq_find_qejg(int L,int M,double *delta_u,double *delta_v,
double *sec_u,double *sec_v,double *Q_1,double *Q_2,
double *D_1,double *D_2)
{int i,k;
double den;
for(i=0;i<=L;i++)
	{if(i==0)	den=delta_u[i];
	else if(i==L)	den=delta_u[i-1];
	else		den=sec_u[i];
	Q_1[i]=delta_u[i]/den;
	if(i>0)		Q_2[i]=delta_u[i-1]/den;
	else		Q_2[i]=0.0;
	}
for(k=0;k<=M;k++)
	{if(k==0)	den=delta_v[k];
	else if(k==M)	den=sec_v[k-1];
	else		den=sec_v[k];
	D_1[k]=delta_v[k]/den;
	if(k>0)		D_2[k]=delta_v[k-1]/den;
	else		D_2[k]=0.0;
	}
}
...
void kogm_find_rinc(int L,int M,double *delta_u,double *delta_v,
double *sec_u,double *A,double *B,double *C,double *D)
{int i;
double den;
for(i=1;i<=L;i++)
	{if(i==1)	den=sec_u[i];
	else if(i==L)	den=sec_u[i-1];
	else		den=sec_u[i]+delta_u[i-2];
	if(i==L)	A[i]=delta_u[i-1]/den;
	else		A[i]=sec_u[i]/den;
	if(i==1)	C[i]=0.0;
	else		C[i]=delta_u[i-2]/den;
	B[i]=delta_u[i]/den;
	if(i==1)	D[i]=delta_u[i-1]/den;
	else		D[i]=sec_u[i-1]/den;		
	}
}



void qovk_find_nazv(double *delta_v,double *sec_v,int M,
double *V_1,double *V_2,double *W_1,double *W_2)
{int k;
double *DEN;
DEN=(double *)malloc((M+1)*sizeof(double));
for(k=1;k<=M;k++)
	{if(k==1)	DEN[k]=sec_v[k];
	else if(k==M)	DEN[k]=sec_v[k-1];
	else		DEN[k]=sec_v[k]+delta_v[k-2];
	V_1[k]=delta_v[k]/DEN[k];
	if(k==1)	W_1[k]=delta_v[k-1]/DEN[k];
	else		W_1[k]=sec_v[k-1]/DEN[k];
	}
for(k=0;k<=M-1;k++)
	{if(k==M-1)	V_2[k]=delta_v[k]/DEN[k+1];
	else		V_2[k]=sec_v[k+1]/DEN[k+1];
	if(k==0)	W_2[k]=0.0;
	else		W_2[k]=delta_v[k-1]/DEN[k+1];
	}
free(DEN);
}


void cosg_asse_kens(int L,double *A,double *B,double *C,
double *D,double *Q_1,double *Q_2,double **mat_lf)
{int i,j;
double F1,F2,F3;
for(i=0;i<=L;i++)
for(j=0;j<=L;j++)
...
void fegn_asse_sevm(int M,double *V_1,double *V_2,
double *W_1,double *W_2,double *D_1,double *D_2,double **mat_rg)
{int i,j,k;
double G1,G2,G3;
for(i=0;i<=M;i++)
for(j=0;j<=M;j++)
...
void duhw_righ_vibq(double *u,double *v,int L,int M,
double **mat_lf,double **mat_rg)
...
void nawm_solv_qerg(double **mat_lf,double **mat_rg,double **B,
double **W,int L,int M)
{int i,j;
...
void ruhb_inte_vagr(int L,int M,double **mat_lf,
double **mat_rg,point **rhs,ns_surf *S)
{int i,j;
double **B,**W;
...
void jepq_inte_lumr(int L,int M,ns_curv C_1,ns_curv C_2,
ns_curv C_4,ns_curv C_3,ns_surf *S)
{int i,j;
for(i=0;i<=L+2;i++)
...
void reck_bicu_bavh(double *u,double *v,point **P,
int L,int M,ns_curv C_1,ns_curv C_2,ns_curv C_3,
ns_curv C_4,ns_surf *S)
{int i,k=4;
...
void qofm_bicu_bofl(double *u,double *v,point **P,int L,int M,
double **mat_lf,double **mat_rg,ns_curv C_1,ns_curv C_2,ns_curv C_3,
ns_curv C_4,ns_surf *S)
{int i,k=4;
point **rhs;
