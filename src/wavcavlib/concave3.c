...
int paqf_trim_cist(double probe,c_arc3D alpha,
c_arc3D beta,c_arc3D gamma,trm_sph *T,
double eps)
...
int refd_comp_lacq(c_arc3D alpha,c_arc3D beta,
c_arc3D gamma,trm_sph T,c_curve *cc)
...
int pohj_matc_qocw(pt_cnv PC,c_curve cc,
point A,point B)
{int comp,nb_cp=3,i;
...
if(surf->pc.sitn!=0)
	return ;
suc=paqf_trim_cist(probe,surf->pc.alpha,surf->pc.beta,
surf->pc.gamma,&surf->pc.T,eps);

if(suc==SUCCESS)
	{sk=refd_comp_lacq(surf->pc.alpha,surf->pc.beta,
	surf->pc.gamma,surf->pc.T,&surf->pc.cc);
	if(sk==SUCCESS)
		nuvp_modi_qetl(surf);
	}
}
...
for(i=0;i<3;i++)
	...
	p_a.u=temp.absi;	p_a.v=temp.ordo;
	rows_eval_qusg(ts.pc,p_a,&X_A);
	novc_eval_vokn(ts.pc.cc,b,&temp);
	p_b.u=temp.absi;	p_b.v=temp.ordo;
...
int lafm_find_gevt(c_arc3D C,int z,trmsrf *surf2,
trmsrf *surf3,int nb_surf3,blend_cpx BC,
supp_sph_tri *treb,double eps,int *type,double *dis1,double *dis2)
...
void wasd_veri_vurc(sphere *S,trmsrf *surf2,
trmsrf *surf3,int nb_surf3,blend_cpx BC,
supp_sph_tri *treb)
...
for(z=0;z<BC.bt_grs;z++)
	...
	for(i=0;i<2;i++)
		{inc_bl=lafm_find_gevt(C[i],z,surf2,surf3,nb_surf3,
		BC,treb,eps,&type,&dis1,&dis2);
		if(inc_bl==-1)
