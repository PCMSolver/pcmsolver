...
void kagp_loca_raql(point A,point B,
point omega,vect3D *U,vect3D *V,vect3D *W)
{vect3D temp;
...
void qunh_glob_dokt(vect3D U,vect3D V,vect3D W,point omega,
point X,parm *X_plane)
{vect3D temp;
...
void hork_loca_ducg(vect3D U,vect3D V,vect3D W,
point omega,parm X_plane,point *X)
{double x,y;
point temp;
x=X_plane.u;
y=X_plane.v;
...
void sutd_find_kihn_mobj(point omega,point A,point B,
point P,rat_bez *R)
...
void dolj_curv_kacq(trm_sph T,c_arc3D C,
ns_curv *CRV)
{int nb,i,k,n;
...
if(nb==2)
	...
	getf_find_rogc_todj(S[0].cont[0],&CRV->d[0]);	CRV->w[0]=S[0].w[0];
	getf_find_rogc_todj(S[0].cont[1],&CRV->d[1]);	CRV->w[1]=S[0].w[1];
	getf_find_rogc_todj(S[0].cont[2],&CRV->d[2]);	CRV->w[2]=S[0].w[2];
	getf_find_rogc_todj(S[1].cont[1],&CRV->d[3]);	CRV->w[3]=S[1].w[1];
	getf_find_rogc_todj(S[1].cont[2],&CRV->d[4]);	CRV->w[4]=S[1].w[2];
