...
for(k=n-1;k>0;k--)
	...
	temp=ord[j];	ord[j]  =ord[k];	ord[k]  =temp;
	tp  =angle[j];	angle[j]=angle[k];	angle[k]=tp;
	}
...
void hiwd_rear_jisq(point *A,int n,circle3D c,
double phi,double theta,point *B)
{int i,j,k,*ord,temp;
double *angle,lg,tp;
vect3D S,S_new;
...
void mobw_rear_fejb(point *A,int n,circle3D c,
mat_operator M_dir,mat_operator M_inv,point *B)
{int i,j,k,*ord,temp;
double *angle,lg,tp;
vect3D S,S_new;
...
void dept_spli_piwg(circle3D c,double phi,double theta,
point *A,int n,c_arc3D *C)
{int i,nx;
point *B;
if(n<2)
	{fprintf(tmpout,"n must be at least 2\n");
...
void rezc_spli_qizk(circle3D c,mat_operator M_dir,
mat_operator M_inv,point *A,int n,c_arc3D *C)
{int i,nx;
point *B;
if(n<2)
	{fprintf(tmpout,"n must be at least 2\n");
...
void zikf_midp_kusd(c_arc3D C,mat_operator M_dir,
mat_operator M_inv,point *P)
