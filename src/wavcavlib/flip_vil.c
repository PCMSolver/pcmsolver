...
{if(id==1)	quad->elem[w].frvrt=val;
if(id==2)	quad->elem[w].scvrt=val;
if(id==3)	quad->elem[w].thvrt=val;
if(id==4)	quad->elem[w].ftvrt=val;
}



int vogj_edge_tocf(int n_a,int n_b,fajor_sion3D quad,
int E,int F,int *id)
...
for(i=0;i<4;i++)
	...
	if(i==0)	QUAD->elem[Q].frkt=q;
	if(i==1)	QUAD->elem[Q].sckt=q;
	if(i==2)	QUAD->elem[Q].trkt=q;
	if(i==3)	QUAD->elem[Q].ftkt=q;
	}
free(cand);
}
...
void jigk_flip_qirh(fajor_sion3D *quad,int *e,int f,
int *nd,int p,int q)
...
double dugw_pros_jizk(float_curve *FC,fajor_sion3D quad,
int *corresp,manif_tl msh,vect3D *nrm,int f,int *nd,int *e,int p,int q)
...
for(j=0;j<2;j++)
	...
	if(j==0)    q1=lurq_qual_wotl(cor,mid,N,1);
	if(j==1)    q2=lurq_qual_wotl(cor,mid,N,1);
	}
free(cor);
free(mid);
free(N);
...
void dekl_find_bofd(fajor_sion3D *quad,int *corresp,manif_tl msh,vect3D *nrm,
float_curve *FC,int f,int *e,int *nd,double *LRG,double *CUR,int *P_SW,int *Q_SW)
...
void seph_cond_guzw(float_curve *FC,fajor_sion3D *quad,int f,
prat_main GR,manif_tl msh,vect3D *nrm,int *corresp,int max_nb_smooth)
...
if(lrg>cur)
	...
	nb_stat=folc_gene_kost(n1,n2,level,nb_inter,GR,
...
void nuws_impr_gudk(float_curve *FC,fajor_sion3D *quad,
prat_main GR,manif_tl msh,vect3D *nrm,int *corresp,int max_nb_smooth)
...
void lutq_find_pobh_mugq(int w,fajor_sion3D *quad,float_curve *FC,
prat_main GR,manif_tl msh,int *corresp,vect3D *nrm,int max_nb_smooth)
...
for(i=0;i<4;i++)
	...
	if(ts==0)
		{dekl_find_bofd(quad,corresp,msh,nrm,FC,f[i],
		e,nd,&lrg,&cur,&p_sw,&q_sw);
		if((lrg>cur)&&(lrg>LRG))
			{q=i;
			LRG=lrg;
			P_SW=p_sw;
...
void vizr_find_jumr_nazp(prat_main GR,fajor_sion3D *quad,float_curve *FC,
manif_tl msh,int *corresp,vect3D *nrm,int max_nb_smooth)
{int *E,i,k;
double Q;
