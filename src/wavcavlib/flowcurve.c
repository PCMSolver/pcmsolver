...
int nubg_smoo_vufb(manif_tl vghwer,prat_main GR,rgb_lk *col,kt *ED,
int NED,int *corresp,float_curve *F,int max_nb_smooth)
...
void kewg_veri_hosp(fajor_sion3D QUAD,int max_nnd_quad,
int max_nel_quad,int max_ned_quad)
...
void hojr_four_coqd(int choic,int ex,manif_tl srbv,manif_tl vghwer,
rgb_lk *col,sphere *SP,int max_nnd,int max_nel,int max_ned,int *forc_term)
...
nb_smooth=nubg_smoo_vufb(vghwer,GR,col,QUAD.kt,NED,
corresp,FC,max_nb_smooth);	
if(1)
...
void runv_refi_pucq(point A,point B,point STR,point TRM,
double lm_1,double lm_2,int N,point *X,double *n_lm_1,double *n_lm_2)
{int i,q;
...
int niqg_refi_kidm(point A,point B,point STR,
point TRM,point GS,int N,int depth,point *X)
...
int lodt_impr_kinw(manif_tl msh,prat_main GR,int k,float_curve *fc,
int max_nb_smooth)
...
nb_stat_loc = folc_gene_kost(N1,N2,level,nb_inter,GR,
msh,omega_loc,type_loc,underl_loc,max_stat_loc);
nb_old=N2-N1+1;
...
void dofr_impr_qovs(manif_tl msh,prat_main GR,
float_curve *fc,int max_nb_smooth)
...
for(i=0;i<nb_fl;i++)
	homs_boun_gosm(fc[i].stn,fc[i].st_grs,&B[i].x_min,
	&B[i].x_max,&B[i].y_min,&B[i].y_max,&B[i].z_min,&B[i].z_max);
nb=0;
for(i=0;i<nb_fl;i++)
for(j=0;j<i;j++)
...
void nedh_heal_rimg(manif_tl msh,float_curve *fc,
prat_main GR,int nb_fl,int max_nb_smooth,int *forc_term)
