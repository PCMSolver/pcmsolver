...
{int res;
if(msh.entity[r].frvrt==s)	res=msh.entity[r].scvrt;
if(msh.entity[r].scvrt==s)	res=msh.entity[r].thvrt;
if(msh.entity[r].thvrt==s)	res=msh.entity[r].frvrt;
return res;
}
...
void taqr_mesh_tuqf(manif_tl msh,prat_main *G,int *root,
int *forc_term)
...
for(i=0;i<ned;i++)
	{if(msh.kt[i].scent!=-1)
		...
		rocq_stat_todr(lambda,msh,msh.kt[i].frent,
		msh.kt[i].scent,&idx,&f_trm);
		if(f_trm==1)
			{*forc_term=1;
			free(P);
			free(map);
...
rt=-1;
for(i=0;i<ned;i++)if(msh.kt[i].scent==-1)
	{s=msh.kt[i].frent;
	f[0]=msh.entity[s].frkt;
	f[1]=msh.entity[s].sckt;
	f[2]=msh.entity[s].trkt;
	for(j=0;j<3;j++)if(msh.kt[f[j]].scent!=-1)
