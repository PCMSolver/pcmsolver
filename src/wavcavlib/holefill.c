...
int rowm_fill_farq(manif_tl msh,int *sub_msh,int N,
int *ind_member,int max_sub)
...
int sufh_find_tadf(manif_tl msh,int *sub_msh,int N,
int *ind_member,int *bound)
...
int vegd_enla_wemf(manif_tl msh,int *bound,int nb,
hash_entry *CC,int max_entries)
...
void befr_enla_dusc(manif_tl msh,int *bound,
int nb,hash_entry *CC,int max_entries)
{int i,suc;
for(i=0;i<2*nb;i++)
...
int huvr_test_cajg(hash_entry *CC,int nb_cc,
int *bound,int nb,int *e)
...
int netv_conn_fatv(manif_tl msh,int *bound,int nb,
hash_entry *CC,int max_entries,int max_cc)
{int nb_cc,i,ts,e;
CC[0].list[0]=bound[0];
CC[0].nb=1;
nb_cc=1;
...
int roch_adva_jusw(manif_tl msh,hash_entry *CC,
int nb_cc,int *sub_msh,int *ind_member,int N,int max_sub)
...
int morn_heal_lowd(manif_tl msh,int *cr,int nb_cr,int *sub_msh,
int *ind_member,int N,int max_sub)
...
int mujl_fill_zuqt(manif_tl msh,int *sub_msh,int N,
int *ind_member,int max_sub)
{int N_nx,N_cur,p;
N_cur=N;
for(p=0;p<3*N;p++)
...
int fivg_adap_qotn(point *P,int nb,point X,
double eps_min,double eps_max,int *id)
{int N=30,res,ts,i;
...
int wukc_find_lugs(float_curve *FC,manif_tl msh,rgb_lk *col,sphere *SP,
int *cv,manif_tl *img,sphere *S_img,int *zoro,int max_nnd_img,
int max_nel_img)
...
sk=nahk_part_kent(FC,msh,col,SP,cv,&sub,col_sub,S_sub,map_node,
map_elem,max_nnd_sub,max_nel_sub);
if(sk==FAILURE)
	SUC=FAILURE;
if(sk==SUCCESS)
