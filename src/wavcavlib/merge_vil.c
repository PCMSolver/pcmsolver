...
void vunm_comm_dilp(fajor_sion3D quad,
int E,int F,int *q1,int *q2)
...
void dezq_surr_qong(fajor_sion3D quad,int E,int F,
int q1,int q2,int *p)
...
void tolm_merg_wovm(fajor_sion3D *quad,int *corresp,
int E,int F,float_curve *fc,int max_nb_smooth)
...
for(i=0;i<4;i++)
	{if(quad->kt[p[i]].frent==F)	quad->kt[p[i]].frent=E;
	if(quad->kt[p[i]].scent==F)	quad->kt[p[i]].scent=E;
	}
free(p);
...
void lujs_find_capm_nerv(fajor_sion3D *quad,int *corresp,
float_curve *fc,int *nb_smooth,int max_nb_smooth)
