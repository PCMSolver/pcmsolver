...
int fugs_find_gisn_jarc(double xmi_1,double xma_1,double ymi_1,
double yma_1,double zmi_1,double zma_1,double xmi_2,double xma_2,
double ymi_2,double yma_2,double zmi_2,double zma_2)
{if((xma_1<xmi_2)||(xma_2<xmi_1))	return 0;
if((yma_1<ymi_2)||(yma_2<ymi_1))	return 0;
if((zma_1<zmi_2)||(zma_2<zmi_1))	return 0;
return 1;
}
...
{int res;
res=fugs_find_gisn_jarc(B1.x_min,B1.x_max,B1.y_min,B1.y_max,
B1.z_min,B1.z_max,B2.x_min,B2.x_max,B2.y_min,B2.y_max,
B2.z_min,B2.z_max);
return res;
}
...
void ritp_boun_niwz(parm *P,int N,double *XMI,
double *XMA,double *YMI,double *YMA)
{int j;
...
yma=-LARGE_NUMBER;
for(j=0;j<N;j++)
	{if(P[j].u<xmi)	xmi=P[j].u;
	if(P[j].u>xma)	xma=P[j].u;
	if(P[j].v<ymi)	ymi=P[j].v;
	if(P[j].v>yma)	yma=P[j].v;
	}
*XMI=xmi;	*XMA=xma;
*YMI=ymi;	*YMA=yma;
}
