...
double zopg_curv_zecg(int k,int n,int r,
double *d,double *tau,double u)
{int j,i;
...
void wogh_curv_juhg(int k,int n,parm *d,
double *tau,double u,parm *sol)
{int j,i,r;
...
void kuqt_eval_webp(ns_curv nc,
double u,parm *sol)
{int p3,kk,nn,i;
parm *D;
kk=nc.k;
...
else if(p1==1)fprintf(tmpout,"\tPlanar curve\n");
if(p2==0)fprintf(tmpout,"\tOpen curve\n");
else if(p2==1)fprintf(tmpout,"\tClosed curve\n");
if(p3==0)fprintf(tmpout,"\tRational B-spline curve\n");
else if(p3==1)fprintf(tmpout,"\tPolynomial B-spline curve\n");
if(p4==0)fprintf(tmpout,"\tNonperiodic curve\n");
else if(p4==1)fprintf(tmpout,"\tPeriodic curve\n");
for(i=0;i<=nn+kk;i++)
	fprintf(tmpout,"\ttau[%d]=%f\n",i,nc.tau[i]);
for(i=0;i<=nn;i++)
