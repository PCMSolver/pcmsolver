...
void vohc_tens_temh(parm *bottom,parm *top,parm *left,
parm *right,int N,int M,manif_ro *msh)
{int i,j,k,shift;
k=0;
for(j=0;j<M;j++)
	{for(i=0;i<N;i++)
...
for(i=0;i<temp.e_grs;i++)
	{n1=temp.entity[i].frvrt;	msh->entity[i].frvrt=map[n1];
	n2=temp.entity[i].scvrt;		msh->entity[i].scvrt=map[n2];
	n3=temp.entity[i].thvrt;		msh->entity[i].thvrt=map[n3];
	}
...
int goch_para_danw(double *tm,int N,
double t,double *T,int idx,double eps)
{int res,i;
double a,b,diff;
a=T[idx];
b=T[idx+1];
...
int ceth_find_pajf(polygon P,double *T,double *tm,
int *map,double eps,double *add)
{int N,i,j,nb,idx,ts;
double t,t_tilde;
N=P.v_grs;
nb=0;
...
void wajr_inse_goql(trmsrf surf,double *tm,int N,double *T,
double *add,int nb,polygon *P)
{int i,j,k,st,qw;
double *temp,*TM;
point tp;
polygon Q;
