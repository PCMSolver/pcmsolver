...
int tipc_deci_supn(int eq,manif_tl *msh,
fund_quad *Q,double *err,double anis,int *tp,int max_ned)
...
if(res==SUCCESS)
	...
	qosr_fill_fedt(msh);
	ruqn_upda_ducz(msh,old_n1,old_n2,inv_node,
	n_inv,map_nd,nnd_old,ned_old,err,Q);
	}
free(map_edge);
free(map_nd);
free(old_n1);
...
int tojb_deci_hegs(int *list,manif_tl *msh,fund_quad *Q,
double *err,double anis,int nb_trials,int max_ned)
{int suc,i,s,pr=STAGNATION,tp,res;
for(i=0;i<nb_trials;i++)if(i<msh->k_grs)
	{s=list[i];
...
void vofp_deci_hers(int *list,manif_tl *msh,
fund_quad *Q,double *err,double anis,
int nb_trials,int max_ned)
...
void fomd_deci_todj(manif_tl vvlfin,manif_tl *mar,double lambda,
double anis,int nb_trials,int max_ned)
