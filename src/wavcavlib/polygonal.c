...
double cutq_curv_vodp(trmsrf ts,c_curve cc,
...
double compute_error(trmsrf ts,c_curve cc,
...
for(k=n;k>0;k--)
	...
	temp=ag[j];  ag[j]=ag[k-1];  ag[k-1]=temp;
	}
...
void forv_find_fojw_pegj(trmsrf ts,c_curve cc,
double *t,int n,int depth,double acc,double *out,int *nb,int N)
{int i,j,k;
double err,*u,*z;
...
void create_polygon(trmsrf ts,polygon *P,
int depth,double acc,int N,double *tm)
...
void kanq_crea_tuqn(trmsrf ts,mult_conn *mc,
int depth,double acc,int N)
{int *m,tempo,nb_ct,i,j,nb,nb_ct_in,wh,k,nin;
double *T,*PS,*T_in,*PS_in,*out,**s,t;
point temp;
