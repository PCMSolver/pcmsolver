...
void hiwq_find_cehq(double *t,int n,double *delta,double *sec,
...
for(i=1;i<=n-1;i++)
	{if(i==1)		den1=sec[i];
	else			den1=delta[i-2]+sec[i];
	if(i==n-1)		den2=sec[i];
	else			den2=delta[i-1]+sec[i+1];
	
	alpha[i]=sqr[i]/den1;
	if(i==1)		a=delta[i]*delta[i-1]/den1;
	else			a=delta[i]*sec[i-1]/den1;
	if(i==n-1)		b=delta[i-1]*delta[i]/den2;
	else			b=delta[i-1]*sec[i+1]/den2;
	beta[i]=a+b;
...
void josg_lins_sung(int n,double *alpha,double *beta,
double *gamma,double **MAT)
{int i,j;
for(i=0;i<=n;i++)
for(j=0;j<=n;j++)
	MAT[i][j]=0.0;
...
void magr_righ_cuws(int n,double *sec,double b_f,
double b_l,double *x,double *r)
{int i;
r[0]=b_f;
r[n]=b_l;
for(i=1;i<n;i++)
...
void homr_find_sowk(double *t,point *X,
point B_F,point B_L,int n,point *D)
{int i;
...
void fumv_find_hotd(double *t,double *X,
double B_F,double B_L,int n,double *D)
{int i;
double *delta,*alpha,*beta,*gamma;
double **MAT,*rhs,*temp,*sol,*sec;
...
void guwn_cubi_pelc(double *t,double *X,
int n,double *D)
{double lambda,mu;
double B_F,B_L;
lambda=1.0/3.0;
