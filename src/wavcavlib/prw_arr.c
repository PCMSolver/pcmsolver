...
void lurp_find_kafg_delc(double marg,int q,manif_tl *msh,msh_corn *m_c,
int msh_id,point **MID,int *forc_term)
...
tuwh_fill_zupf(m_c_old,nnd_old,cr1,cr2,
marg,*msh,m_c,map_node,&f_trm);
if(f_trm==1)
...
double qehs_dist_zugt(int q1,int q2,msh_corn m_c1,
msh_corn m_c2,manif_tl msh1,manif_tl msh2)
{int a,b;
double dis,d1,d2;
point A1,B1,A2,B2;
...
int tanv_asso_qupr(int p,int q,megamanif MG,
int *obsol,msh_corn *M_C,int *p_asc,int *q_asc)
{int i,j,ts,res=FAILURE;
double d,sml;
sml=LARGE_NUMBER;
for(i=0;i<MG.mw_grs;i++)if((i!=p)&&(obsol[i]==0))
...
void cufq_othe_gitl(msh_corn *M_C,
int p_asc,int q_asc,int *q_other)
{int ned,i,pos;
*q_other=-1;
pos=M_C[p_asc].E[q_asc].pos;
ned=M_C[p_asc].k_grs;
for(i=0;i<ned;i++)if(i!=q_asc)
...
int qevp_find_lovn_fejq(megamanif MG,msh_corn *M_C,int p_asc,
int q_asc,int p,int q,int *P_ASC,int *Q_ASC,point **MID)
{int pos,q_oth;
double d1,d2;
point REF;
pos=M_C[p_asc].E[q_asc].pos;
...
int welq_asso_benv(int p,int q,megamanif MG,int *obsol,
msh_corn *M_C,point **MID,int *p_asc,int *q_asc,int **exc)
{int i,j,ts,res=FAILURE;
int P_ASC,Q_ASC;
double d,sml;
sml=LARGE_NUMBER;
for(i=0;i<MG.mw_grs;i++)if((i!=p)&&(obsol[i]==0))
for(j=0;j<M_C[i].k_grs;j++)if(exc[i][j]==0)
	{ts=lafc_boun_gusd(M_C[p].E[q].BB,M_C[i].E[j].BB);
	if(ts==1)
		{d=qehs_dist_zugt(q,j,M_C[p],M_C[i],MG.msh[p],MG.msh[i]);
		if(d<sml)
...
int tesq_path_hegn(manif_tl msh,int nd1,int nd2,int pos,
msh_corn m_c,conn_seq C,int *pth)
...
for(i=0;i<N;i++)
	...
	,MG.msh[m].knot[q].absi,MG.msh[m].knot[q].ordo,MG.msh[m].knot[q].cote);
	}
}
...
int repv_find_mowl_vurw(megamanif MG,incident_gnode **CG,int n_corn,
msh_corn *M_C,fam_nodes *F,int max_fam,int max_memb)
...
for(i=0;i<n_corn;i++)
	...
	f_loc[0].msh_id[0]=p0;	f_loc[0].knot_id[0]=a0;
	f_loc[0].msh_id[1]=p1;	f_loc[0].knot_id[1]=a1;
	f_loc[0].n_grs=2;
	f_loc[1].msh_id[0]=p0;	f_loc[1].knot_id[0]=b0;
	f_loc[1].msh_id[1]=p1;	f_loc[1].knot_id[1]=b1;
	f_loc[1].n_grs=2;
...
int pulw_find_rukq_gojn(megamanif MG,msh_corn *M_C,incident_gnode **CG,
conn_seq *C,int n_inc,fam_nodes *F,int *forc_term)
...
nb=0;
for(i=0;i<n_inc;i++)
	{p0=CG[i][0].msh_id;	q0=CG[i][0].kt_id;
	p1=CG[i][1].msh_id;		q1=CG[i][1].kt_id;
	somn_take_dogm(q0,M_C[p0],&a0,&b0);
	somn_take_dogm(q1,M_C[p1],&a1,&b1);
	...
	for(j=0;j<L;j++)
		...
		n0=pth0[j+1];
		n1=pth1[j+1];
		F[nb].msh_id[0]=p0;	  F[nb].knot_id[0]=n0;
		F[nb].msh_id[1]=p1;	  F[nb].knot_id[1]=n1;
...
void qujs_loca_wilr(fam_nodes *F_corn,int n_prw,
fam_nodes *F_edge,int n_edge,megamanif MG,int pt,supp_surf *sp_sr,
supp_surf *ssr,manif_tl *msh)
...
for(i=0;i<n_prw;i++)
	{M=F_corn[i].n_grs;
	for(j=0;j<M;j++)if(F_corn[i].msh_id[j]==pt)
...
for(i=0;i<n_edge;i++)
	{M=F_edge[i].n_grs;
	for(j=0;j<M;j++)if(F_edge[i].msh_id[j]==pt)
...
void sukt_comb_bejq(megamanif MG,int *obsol,fam_nodes *F_corn,
int n_prw,fam_nodes *F_edge,int n_edge,supp_surf *sp_sr,
supp_surf *ssr,manif_tl *msh)
{int i,k,p,n_msh;
k=0;
for(i=0;i<n_prw;i++)
...
msh->e_grs=0;
n_msh=MG.mw_grs;
for(p=0;p<n_msh;p++)if(obsol[p]==0)
	qujs_loca_wilr(F_corn,n_prw,F_edge,n_edge,MG,p,sp_sr,ssr,msh);
}


void zekc_merg_gelw(megamanif *MG,msh_corn *M_C,point **MID,
supp_surf *sp_sr,supp_surf *ssr,rgb_lk *col,int max_ned,
manif_tl *msh,int *forc_term)
...
for(i=0;i<n_msh;i++)if(obsol[i]==0)
...
k=0;
for(i=0;i<MG->mw_grs;i++)if(obsol[i]==0)
