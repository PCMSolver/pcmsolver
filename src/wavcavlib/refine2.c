...
int rudw_loca_ragm(point *A,point *s,point *t,int *type_s,
int *type_t,int N,manif_tl *loc)
...
void vugf_find_legt_sokl(double **mat_dir,double *trans_dir,
double **mat_inv,double *trans_inv)
{int i;
double det;
for(i=0;i<2;i++)
...
void suvd_prei_walj(map_hyper M,point P,
double *u,double *v)
{vect3D temp;
...
int fils_loca_poth(point *A,point *S,point *T,int *type_s,
int *type_t,int N,manif_tl *loc)
