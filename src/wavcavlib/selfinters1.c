...
void rehp_find_deng(point *P,int n,int id_0,
int id_1,int id_2,int *idx)
{int i,i_sp,dummy,ts;
double lg,ds;
idx[0]=id_0;
idx[1]=id_1;
...
void qumg_circ_tezl(point a,point b,point c,point d,
double *rad,point *center)
...
int qojc_reco_fadc(point *P,int n,int id_0,int id_1,
int id_2,double eps,point *center,double *rad)
...
void kotr_find_tujg(manif_ro msh_tri,int id_0,int id_1,
int id_2,pt_cnv PC,conc_model *CM)
...
nnd=CM->msh.n_grs;
nel=CM->msh.e_grs;
homs_boun_gosm(CM->msh.knot,nnd,&CM->BB.x_min,&CM->BB.x_max,
&CM->BB.y_min,&CM->BB.y_max,&CM->BB.z_min,&CM->BB.z_max);
...
for(i=0;i<nel;i++)
	...
	U3D.absi=U.u;	V3D.absi=V.u;
	U3D.ordo=U.v;	V3D.ordo=V.v;
	U3D.cote=0.0;	V3D.cote=0.0;
...
for(i=0;i<nnd;i++)
	...
	if(ts==0)
		...
		map[i]=id;
	if(i==cr_a)	*id_0=map[i];
	if(i==cr_b)	*id_1=map[i];
	if(i==cr_c)	*id_2=map[i];
	}
msh->n_grs=k;
