...
int zacr_find_netw_wusf(int s,prat_main GR,manif_tl msh,int level,
int *ngb,int max_size)
...
int cahq_idea_fewg(fajor_sion3D quad,int *corresp,
prat_main GR,manif_tl msh,int lev,int nd_a,int nd_b)
...
void ripn_find_reln(fajor_sion3D quad,int F,int nd_a,int nd_b,
int *m_1,int *m_2,int *e_a,int *e_b,int *f_a,int *f_b)
...
int minf_invo_jawc(fajor_sion3D quad,int nd_a,int nd_b,
int e_a,int f_a,int e_b,int f_b,int *inv_ed,int max_inv)
{int ned,i,n1,n2,N;
ned=quad.k_grs;
N=0;
for(i=0;i<ned;i++)
...
void jeqg_skit_leqb(fajor_sion3D *quad,manif_tl msh,prat_main GR,
int F,int nd_a,int nd_b,int id,int *corresp,float_curve *fc,
int nb_inter,int max_nb_smooth)
...
for(i=0;i<quad->e_grs;i++)
	...
	e1=quad->elem[i].frkt;	if(e1==f_a)		quad->elem[i].frkt=e_a;
	e2=quad->elem[i].sckt;	if(e2==f_a)		quad->elem[i].sckt=e_a;
	e3=quad->elem[i].trkt;	if(e3==f_a)		quad->elem[i].trkt=e_a;
	e4=quad->elem[i].ftkt;	if(e4==f_a)		quad->elem[i].ftkt=e_a;
	
	e1=quad->elem[i].frkt;	if(e1==f_b)		quad->elem[i].frkt=e_b;
	e2=quad->elem[i].sckt;	if(e2==f_b)		quad->elem[i].sckt=e_b;
	e3=quad->elem[i].trkt;	if(e3==f_b)		quad->elem[i].trkt=e_b;
	e4=quad->elem[i].ftkt;	if(e4==f_b)		quad->elem[i].ftkt=e_b;
	}
...
if(quad->kt[e_a].frent==F)	inc_e=quad->kt[e_a].scent;
else						inc_e=quad->kt[e_a].frent;
if(quad->kt[f_a].frent==F)	inc_f=quad->kt[f_a].scent;
else						inc_f=quad->kt[f_a].frent;
...
if(quad->kt[e_b].frent==F)	inc_e=quad->kt[e_b].scent;
else						inc_e=quad->kt[e_b].frent;
if(quad->kt[f_b].frent==F)	inc_f=quad->kt[f_b].scent;
else						inc_f=quad->kt[f_b].frent;
quad->kt[e_b].frent=inc_e;
quad->kt[e_b].scent=inc_f;

for(i=0;i<quad->k_grs;i++)
	{n1=quad->kt[i].frvrt;	if(n1==nd_b)	quad->kt[i].frvrt=nd_a;
	n2=quad->kt[i].scvrt;		if(n2==nd_b)	quad->kt[i].scvrt=nd_a;
	}
...
free(temp_ed);
for(i=0;i<quad->e_grs;i++)
	{e1=quad->elem[i].frkt;	quad->elem[i].frkt=map_ed[e1];
	e2=quad->elem[i].sckt;	quad->elem[i].sckt=map_ed[e2];
	e3=quad->elem[i].trkt;	quad->elem[i].trkt=map_ed[e3];
	e4=quad->elem[i].ftkt;	quad->elem[i].ftkt=map_ed[e4];
	}
new_e_a=map_ed[e_a];
new_e_b=map_ed[e_b];
for(i=0;i<n_inv;i++)
...
free(temp_qd);
for(i=0;i<quad->k_grs;i++)
	{e1=quad->kt[i].frent;	quad->kt[i].frent=map_qd[e1];
	e2=quad->kt[i].scent;		
	if(e2!=-1)
...
free(temp_nd);
for(i=0;i<quad->e_grs;i++)
	{n1=quad->elem[i].frvrt;	quad->elem[i].frvrt=map_nd[n1];
	n2=quad->elem[i].scvrt;	quad->elem[i].scvrt=map_nd[n2];
	n3=quad->elem[i].thvrt;	quad->elem[i].thvrt=map_nd[n3];
	n4=quad->elem[i].ftvrt;	quad->elem[i].ftvrt=map_nd[n4];
	}
for(i=0;i<quad->k_grs;i++)
	{n1=quad->kt[i].frvrt;	quad->kt[i].frvrt=map_nd[n1];
	n2=quad->kt[i].scvrt;		quad->kt[i].scvrt=map_nd[n2];
	}
free(map_nd);
...
for(i=0;i<n_inv;i++)
	{n1=quad->kt[inv_ed[i]].frvrt;	im_n1=corresp[n1];
	n2=quad->kt[inv_ed[i]].scvrt;		im_n2=corresp[n2];
	nb_stat=folc_gene_kost(im_n1,im_n2,level,nb_inter,GR,
	msh,omega,type,underl,max_nb_smooth);
...
void jecr_find_zemc_wozf(fajor_sion3D quad,int nd,int z,
float_curve *f,int *e,int *ort)
...
int moqr_reco_lecn(fajor_sion3D quad,
int F,float_curve *fc,int *nd_a,int *nd_b)
...
void tuqk_trav_nesv(fajor_sion3D *quad,manif_tl msh,prat_main GR,
int *corresp,float_curve *fc,int *nb_smooth,int max_nb_smooth)
...
for(i=0;i<quad->e_grs;i++)
	...
	if(ts==1)
		...
		jeqg_skit_leqb(quad,msh,GR,F,nd_a,nd_b,id,corresp,fc,
		nb_inter,max_nb_smooth);
		M=M-2;
		}
	}
*nb_smooth=M;
