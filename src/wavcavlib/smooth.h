...
void reck_bicu_bavh(double *, double *, point **, int, int, ns_curv, ns_curv, ns_curv, ns_curv, ns_surf *);
void qofm_bicu_bofl(double *, double *, point **, int, int, double **, double **, ns_curv, ns_curv, ns_curv, ns_curv, ns_surf *);
void tesr_colo_donr(int, double *, double *, double *);
void seph_cond_guzw(float_curve *, fajor_sion3D *, int, prat_main, manif_tl, vect3D *, int *, int);
...
int wukc_find_lugs(float_curve *, manif_tl, rgb_lk *, sphere *, int *, manif_tl *, sphere *, int *, int, int);
void lesm_find_vung(ns_surf *, int, int, int, PL_curve *);
void vizr_find_jumr_nazp(prat_main, fajor_sion3D *, float_curve *, manif_tl, int *, vect3D *, int);
...
int dagt_find_tunv_letr(manif_ro, bd_box2D *, manif_tl, sphere * S, int, int, double *, double *, point **);
void nedh_heal_rimg(manif_tl, float_curve *, prat_main, int, int, int *);
void mehk_find_godk_lakt(int, double);
void nuws_impr_gudk(float_curve *, fajor_sion3D *, prat_main, manif_tl, vect3D *, int *, int);
void hopf_impr_zugw(fajor_sion3D *, manif_tl, vect3D *, int *, prat_main, float_curve *, int);
...
int nahk_part_kent(float_curve *, manif_tl, rgb_lk *, sphere *, int *, manif_tl *, rgb_lk *, sphere *, int *, int *, int, int);
void forn_proj_qukz(sphere, point, point *);
int picn_find_vezj_pazq(double **, double **, int, int *, ns_curv *, manif_tl *, sphere *, point *, int *, ns_surf *);
...
void tuqk_trav_nesv(fajor_sion3D *, manif_tl, prat_main, int *, float_curve *, int *, int);
int wuhn_trim_qern(float_curve *, manif_tl, rgb_lk *, sphere *, manif_tl *, sphere *, int *, int, int);
double lurq_qual_wotl(point *, point *, vect3D *, int);
double rohl_qual_zifh(int, point, vect3D, fajor_sion3D, int *, manif_tl, vect3D *, float_curve *, hash_entry *);
...
int gapw_smoo_pogh(double **, double **, int, manif_ro, manif_tl, sphere *, ns_curv *, ns_surf *);
...
void satn_find_selt_jufs(megamanif, PL_curve *, int, point, point);
