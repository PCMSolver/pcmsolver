...
int luct_boun_qifd(float_curve *FC,
int *cv,int *trav,int max_trav)
{int nb,i,j,z,nb_st;
nb=0;
for(i=1;i<=4;i++)
	{z=cv[i];
...
int sotg_expa_hitc(manif_tl msh,int *sub_msh,int *ind_member,int N,
int *trav,int nb_trav,int **exc_ed,int max_sub,int *suc)
...
for(i=0;i<N;i++)
	...
	for(j=0;j<3;j++)if(exc_ed[i][j]==0)
		{f=ed[j];
		e[0]=msh.kt[f].frent;
		e[1]=msh.kt[f].scent;	
		for(k=0;k<2;k++)if((e[k]!=-1)&&(e[k]!=s))
...
int remj_enla_lohp(manif_tl msh,int *sub_msh,int *ind_member,
int N,int max_sub,int *suc)
...
int rukm_expa_cern(int seed,manif_tl msh,int *trav,int nb_trav,
int *sub_msh,int *ind_member,int max_rec,int **exc_ed,
int max_sub,int *N)
...
nel=msh.e_grs;
for(j=0;j<nel;j++)
	{nb_new=sotg_expa_hitc(msh,sub_msh,ind_member,nb,
	trav,nb_trav,exc_ed,max_sub,&suc);
	nb=nb_new;
	if(suc==FAILURE)
		break;
	if(nb>=max_rec)
...
void limr_simp_wufg(manif_tl msh,int *trav,int nb_trav,
int *sub_msh,int *ind_member,int max_sub,int *NB)
...
int tedk_mail_wetr(manif_tl msh,int *trav,int nb_trav,
int *sub_msh,int *ind_member,int max_sub,int *NB)
...
ind=1;
for(i=0;i<nb_sd;i++)if(exc_seed[i]==0)
	{for(p=0;p<max_sub;p++)
	for(q=0;q<3;q++)
		exc_ed[p][q]=0;
	for(p=0;p<msh.e_grs;p++)
		ind_member[p]=0;
	suc=rukm_expa_cern(seed[i],msh,trav,nb_trav,sub_msh,ind_member,
	max_rec,exc_ed,max_sub,&N);
	if(suc==SUCCESS)
		{ind=2;
		break;
		}
	for(j=0;j<nb_sd;j++)if(exc_seed[j]==0)
...
int husm_elem_gumn(float_curve *FC,int *cv,manif_tl msh,
int *sub_msh,int *ind_member,int max_sub,int *NB)
...
int nahk_part_kent(float_curve *FC,manif_tl msh,rgb_lk *col,
sphere *S,int *cv,manif_tl *sub,rgb_lk *col_sub,sphere *S_sub,int *map_node,
int *map_elem,int max_node_sub,int max_elem_sub)
...
void vafh_corr_monz(manif_tl sub,int i,float_curve fc_in,
float_curve *fc_out,int *map_node)
{int nd,ts,id;
nd=fc_in.nd_idx[i];
...
void qenc_corr_tunq(manif_tl msh,manif_tl sub,int i,float_curve fc_in,
float_curve *fc_out,int *map_node)
...
void vegn_dedu_begc(manif_tl msh,manif_tl sub,float_curve fc_in,
int *map_node,int *map_elem,float_curve *fc_out)
{int N,i,cs,z,ts,id;
N=fc_in.st_grs;
for(i=0;i<N;i++)
...
int topr_exte_jemt(manif_tl msh,int *sub_msh,int N,
int *fence,int nb_fence,int *suc)
...
if(n_bound!=nb_fence)
	...
	for(i=0;i<n_bound;i++)
		...
		n_b=im_2[i];
		ind=0;
		for(j=0;j<nb_fence;j++)if(exc[j]==0)
			{z=fence[j];
			n1=msh.kt[z].frvrt;
			n2=msh.kt[z].scvrt;
...
int ticq_trim_pehf(float_curve *fc,manif_tl msh,
rgb_lk *col_msh,int *sub_msh,int *NB)
...
int wuhn_trim_qern(float_curve *fc,manif_tl msh,rgb_lk *col_msh,sphere *S_msh,
manif_tl *img,sphere *S_img,int *zoro,int max_nnd_img,int max_nel_img)
...
if(suc==SUCCESS)
	{map_elem=(int *)malloc(nel*sizeof(int));
	map_node=(int *)malloc(3*nel*sizeof(int));
	tugw_extr_vuzf(msh,img,sub_msh,nb,map_node,
	map_elem,max_nnd_img,max_nel_img);
	for(i=0;i<4;i++)
		{z=zoro[i];
