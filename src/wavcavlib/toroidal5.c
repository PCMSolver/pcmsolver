...
void ruvg_find_miwq(int N,double *t,sphere S1,sphere S2,
point omega,circle3D C1,circle3D C2,point *P1,point *P2,jointbez *J)
{int i,j,deg=2;
...
V.u=delta;	V.v=-d;
W.u=1.0;	W.v=0.0;
cont[0].u=a;	cont[0].v=r1;
cont[2].u=0;	cont[2].v=eps;
D=(eps-r1)/V.v;
cont[1].u=a+D*V.u;
cont[1].v=r1+D*V.v;
for(i=0;i<=deg;i++)
...
W.u=delta;	W.v=-d;
cont[0].u=0;	cont[0].v=eps;
cont[2].u=b;	cont[2].v=r2;
D=(eps-r2)/W.v;
cont[1].u=b+D*W.u;
cont[1].v=r2+D*W.v;
for(i=0;i<=deg;i++)
...
void kegv_find_wild_lodt(int N,sphere S1,sphere S2,
point omega,circle3D C1,circle3D C2,blend_nonself *BN)
{int i,deg=2,nx;
...
void zisq_find_cowk_zevq(int N,sphere S1,sphere S2,
point omega,circle3D C1,circle3D C2,blend_nonself *BN)
{double sp;
circle3D C2_temp;
...
void nowd_eval_fewc(jointbez al,c_arc3D bt,
jointbez gm,c_arc3D dt,double u,double v,point *sol)
