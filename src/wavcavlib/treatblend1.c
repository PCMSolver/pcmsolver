...
int jish_inci_gelh(sphere *S,trmsrf *surf,
blend_cpx BC,int z,int *side_1,int *side_2,double eps)
...
if(suc==SUCCESS)
	{if(q1==0)	sd_1=ON_ALPHA;
	if(q1==1)	sd_1=ON_BETA;
	if(q1==2)	sd_1=ON_GAMMA;
	if(q1==3)	sd_1=ON_DELTA;
	p2=BC.BT[z].sph_idx2;
...
if(suc==SUCCESS)
	{if(q2==0)	sd_2=ON_ALPHA;
	if(q2==1)	sd_2=ON_BETA;
	if(q2==2)	sd_2=ON_GAMMA;
	if(q2==3)	sd_2=ON_DELTA;
...
void kecv_mesh_honc(int side_1,int side_2,int n_width_bl,
int n_len,manif_ro *msh,int *corner)
{int N,M;
...
{if(typ==Q_TYPE)	fprintf(tmpout,"Q_TYPE\n");
if(typ==T_TYPE)		fprintf(tmpout,"T_TYPE\n");
}



int zawp_find_lujp(trmsrf *surf2,trmsrf *surf3,
prat_main_blend G,int seed,double ideal_len)
{int n1,sd1,w,nb,typ;
c_arc3D C;
n1=G.E[seed].frvrt;
sd1=G.E[seed].side_1;
...
int pubd_spre_pedc(trmsrf *surf2,trmsrf *surf3,
prat_main_blend G,int *spec,int *discr_param,double ideal_length)
{int seed,i,j,ned,nd_1,nd_2,dsc,suc,SUC=FAILURE,typ;

ned=G.k_grs;
seed=-1;
for(i=0;i<ned;i++)if(spec[i]==-1)
	{nd_1=G.E[i].frvrt;
	nd_2=G.E[i].scvrt;
...
homs_boun_gosm(P,k,&B->x_min,&B->x_max,
&B->y_min,&B->y_max,&B->z_min,&B->z_max);
free(P);
...
double jufq_dist_dunt(side_flag SF,pt_tor PT,
c_arc3D C,int *side,int *found_id)
{int i,q,*excl;
double dis,res;
c_arc3D *CA;
...
if(SF.aflg==-1)	excl[0]=0;
else					excl[0]=1;
if(SF.bflg==-1)	excl[1]=0;
else					excl[1]=1;
if(SF.gflg==-1)	excl[2]=0;
else					excl[2]=1;
if(SF.dflg==-1)	excl[3]=0;
else					excl[3]=1;
res=LARGE_NUMBER;
q=-1;	*found_id=0;
for(i=0;i<4;i++)if(excl[i]==0)
...
free(excl);
free(CA);
if(q==0)	*side=ON_ALPHA;
if(q==1)	*side=ON_BETA;
if(q==2)	*side=ON_GAMMA;
if(q==3)	*side=ON_DELTA;
return res;
}



double wujt_dist_kaqt(side_flag SF,pt_cnv PC,
c_arc3D C,int *side,int *found_id)
{int i,q,*excl;
double dis,res;
c_arc3D *CA;
