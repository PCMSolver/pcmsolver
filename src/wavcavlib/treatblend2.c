...
void tahq_find_jimp(trmsrf *surf2,trmsrf *surf3,
prat_main_blend G,double ideal_length,int *discr_param)
...
void nohv_find_zatg_meqt(pt_tor PT,int side_1,
int side_2,c_arc3D *C)
{int q,i;
for(i=0;i<4;i++)
...
if(q==0)	poms_find_resk_lonb(PT.alpha,C);
if(q==1)	poms_find_resk_lonb(PT.beta,C);
if(q==2)	poms_find_resk_lonb(PT.gamma,C);
if(q==3)	poms_find_resk_lonb(PT.delta,C);
}


void vowc_veri_cotq(trmsrf *surf2,trmsrf *surf3,
prat_main_blend G)
...
for(i=0;i<ned;i++)
	...
	if(tp1==Q_TYPE)
		{if(sd1==ON_ALPHA)  poms_find_resk_lonb(surf2[w1].pt.alpha,&CA1);
		if(sd1==ON_BETA)    poms_find_resk_lonb(surf2[w1].pt.beta ,&CA1);
		if(sd1==ON_GAMMA)   poms_find_resk_lonb(surf2[w1].pt.gamma,&CA1);
		if(sd1==ON_DELTA)   poms_find_resk_lonb(surf2[w1].pt.delta,&CA1);
		nb_arcs=4;
		}
	if(tp1==T_TYPE)
		{if(sd1==ON_ALPHA)  poms_find_resk_lonb(surf3[w1].pc.alpha,&CA1);
		if(sd1==ON_BETA)    poms_find_resk_lonb(surf3[w1].pc.beta,&CA1);
		if(sd1==ON_GAMMA)   poms_find_resk_lonb(surf3[w1].pc.gamma,&CA1);
		nb_arcs=3;
		}
	
	if(tp2==Q_TYPE)
		{if(sd2==ON_ALPHA)  poms_find_resk_lonb(surf2[w2].pt.alpha,&CA2);
		if(sd2==ON_BETA)    poms_find_resk_lonb(surf2[w2].pt.beta,&CA2);
		if(sd2==ON_GAMMA)   poms_find_resk_lonb(surf2[w2].pt.gamma,&CA2);
		if(sd2==ON_DELTA)   poms_find_resk_lonb(surf2[w2].pt.delta,&CA2);
		nb_arcs=4;
		}
	if(tp2==T_TYPE)
		{if(sd2==ON_ALPHA)  poms_find_resk_lonb(surf3[w2].pc.alpha,&CA2);
		if(sd2==ON_BETA)    poms_find_resk_lonb(surf3[w2].pc.beta ,&CA2);
		if(sd2==ON_GAMMA)   poms_find_resk_lonb(surf3[w2].pc.gamma,&CA2);
		nb_arcs=3;
		}
	...
	if(dis>eps_err)
		...
		if(tp1==Q_TYPE)	fprintf(tmpout,"type1=Q_TYPE\n");
		if(tp1==T_TYPE)	fprintf(tmpout,"type1=T_TYPE\n");
		if(tp2==Q_TYPE)	fprintf(tmpout,"type2=Q_TYPE\n");
		if(tp2==T_TYPE)	fprintf(tmpout,"type2=T_TYPE\n");
...
void muhs_disc_kesq(sphere *S,trmsrf ts,
trmsrf *surf2,blend_cpx BC,int z,set_arcs SA,
int *n_len,int *dis_ex,int **dis_in,int *endp,arc_supp A)
...
int juzv_mult_gehv(trmsrf surf,int *dis_ex,
int **dis_in,mult_conn *P,add_mc *P_app,int *corner,
...
int mult_conn_simple(trmsrf surf,int *dis_ex,
int **dis_in,mult_conn *P,add_mc *P_app,int max_nvt)
{int i,j,k,comp,N,M,nin,p,k_in;
int nb_cr,k_e,n_cur,beg;
double a,b,step,t;
point temp;
...
int salr_test_jofl(trmsrf ts,sphere S,
