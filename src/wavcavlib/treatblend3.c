...
for(i=0;i<nel;i++)
	...
	for(p=0;p<3;p++)
		...
		if(s==-1)
			{N++;
			ed[k].frvrt=N_A[p];   ed[k].scvrt=N_B[p]; 
			ed[k].frent=i;
			ed[k].scent=-1;
			T[i][j]=k; j++;
			k++;
...
int jitr_test_pedr(kt *ed,int sn,int tn,
hash_entry *H,int *flag)
{int p,q,i,N;
*flag=-1;
N=H[sn].nb;
for(p=0;p<N;p++)
...
for(i=0;i<nel;i++)
	...
	for(p=0;p<3;p++)
		...
		if(s==-1)
			{N++;
			ed[k].frvrt=n_st[p];   ed[k].scvrt=n_tr[p]; 
			ed[k].frent=i;
			ed[k].scent=-1;
			T[i][j]=k; j++;
...
void fars_stru_qitd(adj_hash H,atom *S,int z,set_arcs *SA,
trmsrf *surf2,blend_cpx BC,int *forc_term)
...
double fojn_dist_kerf(point S,point T,pt_tor PT,
c_arc3D *C)
{int i,q;
double dis,sml;
c_arc3D *CA;
...
int dent_exis_meqj(point A,point B,int z,blend_cpx BC,
trmsrf *surf2,double eps,int *proc_term)
{int val,i,q,id,res;
double dis,sml;
c_arc3D dummy;
*proc_term=0;
...
void wokc_veri_bopt(blend_cpx BC,int w,trmsrf *surf1,
trmsrf *surf2,int *supp,double eps,int *forc_term)
...
void zevt_stru_copb(adj_hash H,atom *S,int nb_sph,
set_arcs *SA,int *supp,trmsrf *surf1,int nb_surf1,
trmsrf *surf2,blend_cpx BC,int *forc_term)
{int z,w,f_trm;
