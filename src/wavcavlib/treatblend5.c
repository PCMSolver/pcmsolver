...
{int res=-1;
if(T.frvrt==s)		res=T.scvrt;
else if(T.scvrt==s)	res=T.thvrt;
else if(T.thvrt==s)	res=T.frvrt;
return res;
}
...
if(ts==0)
	...
	msh.entity[s].scvrt,msh.entity[s].thvrt);
	fprintf(tmpout,"edges=[%d,%d,%d]\n",msh.entity[s].frkt,
	msh.entity[s].sckt,msh.entity[s].trkt);
...
for(i=0;i<msh->k_grs;i++)
	...
	if(e2!=-1)
		...
		if(ort==0)
			...
			fprintf(tmpout,"element[%d]=[%d,%d,%d]   ",e1,
			msh->entity[e1].frvrt,msh->entity[e1].scvrt,msh->entity[e1].thvrt);
			fprintf(tmpout,"element[%d]=[%d,%d,%d]\n",e2,
			msh->entity[e2].frvrt,msh->entity[e2].scvrt,msh->entity[e2].thvrt);
			exit(0);
			}
		}
	}
...
void jizw_allo_vipg(trmsrf *surf1,int nb_surf1,
int nb_surf2,int nb_surf3,int mx_edge_m,prat_main_m *GM)
{int i,k,nb_msh,nb_c;
...
void kulf_dest_lajs(int nb_surf1,int nb_surf2,
int nb_surf3,int mx_edge_m,prat_main_m *GM)
{int i,nb_msh;
nb_msh=nb_surf1+nb_surf2+nb_surf3;
for(i=0;i<nb_msh;i++)
	free(GM->N[i].corner);
...
void somt_find_qoph_hegd(manif_tl msh,rgb_lk *col,trmsrf *surf1,
trmsrf *surf2,trmsrf *surf3,supp_surf *ssr)
...
for(i=0;i<nel;i++)
	...
	msh.entity[i].scvrt,msh.entity[i].thvrt,col[i].red,col[i].green,col[i].blue,x,y,z,r);
	}
fclose(fp);
err=err/(double)nb;
...
void qazf_popu_fawj(manif_tl msh,rgb_lk *col,trmsrf *surf1,
trmsrf *surf2,trmsrf *surf3,supp_surf *ssr,manif_tl *clmns,
rgb_lk *col_f,sphere *S)
{int nel,i,tp,id,w,n1,nb;
double x,y,z,r,dis,diff,err;
nel=msh.e_grs;
err=0.0;	nb=0;
