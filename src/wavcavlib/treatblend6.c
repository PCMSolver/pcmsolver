...
int jerl_conn_bukc(manif_tl msh,int *mu,int nb,
int seed,int *seq,int *exc)
...
void hojp_boun_hapw(manif_tl msh,conn_seq *C,
int max_conn)
...
void hefn_veri_sezm(megamanif MG,msh_corn *M_C,
int p,int q,int p_asc,int q_asc)
{int a,b,a_asc,b_asc;
double d;
...
int wuvp_inci_tejg(megamanif MG,int *obsol,
msh_corn *m_c,point **MID,incident_gnode **CG)
...
for(p=0;p<n_msh;p++)if(obsol[p]==0)
	...
	for(q=0;q<ned;q++)if(exc[p][q]==0)
		...
		exc[p][q]=1;
		exc[p_asc][q_asc]=1;
		CG[nb][0].msh_id=p;		 CG[nb][0].kt_id=q;
		CG[nb][1].msh_id=p_asc;	 CG[nb][1].kt_id=q_asc;
		nb++;
		}
	}
for(i=0;i<n_msh;i++)
...
int tamb_tigh_joqm(msh_corn m_c,manif_tl msh,double eps,
int *q1,int *q2)
{int q,a,b,ts,*st,res;
point A,B;
...
void tuwq_disc_nafp(double marg,msh_corn *M_C,
point **MID,megamanif *MG,int *obsol,double eps_area,
double eps_fuse,int *forc_term)
	...
	if(ts==1)
		...
		if(tr==1)
			...
			for(j=0;j<2;j++)
				...
				lurp_find_kafg_delc(marg,q_res,&MG->msh[p_res],
				&M_C[p_res],p_res,MID,&f_trm);
				if(f_trm==1)
					{*forc_term=1;
...
void hucz_fuse_qorw(int cr1,int cr2,manif_tl *msh,
int *map_node,int *map_elem)
...
void munk_upda_vugz(int cr1,int cr2,int *map_node,
int pos,msh_corn *M_C)
...
int dejh_find_rohf(int p1,int p2,
int n1_old,int n2_old,int cr1,int cr2)
{int ind=2;
...
int rukl_find_sewj_hodc(int cr1,int cr2,msh_corn M_C,
int *inv_node,int n1_new,int n2_new)
...
void tuwh_fill_zupf(msh_corn m_c_old,int nnd_old,
int cr1,int cr2,double marg,manif_tl msh,msh_corn *m_c,
int *map_node,int *proc_term)
