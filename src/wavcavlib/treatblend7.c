...
{if(z==ON_ALPHA)		return SF.aflg;
else if(z==ON_BETA)		return SF.bflg;
else if(z==ON_GAMMA)	return SF.gflg;
else if(z==ON_DELTA)	return SF.dflg;
else
...
{if(z==ON_ALPHA)		SF->aflg=value;
else if(z==ON_BETA)		SF->bflg =value;
else if(z==ON_GAMMA)	SF->gflg=value;
else if(z==ON_DELTA)	SF->dflg=value;
else
	{fprintf(tmpout,"None of the four cases\n");
	exit(0);
	}
...
int hogm_find_givl_kocf(side_flag *SF,trmsrf *surf2,
trmsrf *surf3,int w,int sd,prat_main_blend G,bd_box3D *B,
int *exc,int *SD,double eps_inc)
...
if(sd==ON_ALPHA)	poms_find_resk_lonb(surf2[z_w].pt.alpha,&C);
if(sd==ON_BETA)		poms_find_resk_lonb(surf2[z_w].pt.beta ,&C);
if(sd==ON_GAMMA)	poms_find_resk_lonb(surf2[z_w].pt.gamma,&C);
if(sd==ON_DELTA)	poms_find_resk_lonb(surf2[z_w].pt.delta,&C);
...
int inc_node_blend_graph_interface(side_flag *SF,trmsrf *surf2,
trmsrf *surf3,int w,int sd,prat_main_blend G,bd_box3D *B,
int *exc,int *SD,double eps_inc)
{int nnd,ts_interf,z_w,z_i;
int i,side,nd,found_id;
...
if(sd==ON_ALPHA)	poms_find_resk_lonb(surf3[z_w].pc.alpha,&C);
if(sd==ON_BETA)		poms_find_resk_lonb(surf3[z_w].pc.beta ,&C);
if(sd==ON_GAMMA)	poms_find_resk_lonb(surf3[z_w].pc.gamma,&C);
...
int wumq_find_gopc_cunw(side_flag *SF,trmsrf *surf2,
trmsrf *surf3,int w,int sd,prat_main_blend G,bd_box3D *B,
int *exc,int *SD,double eps_inc)
...
void jevm_trea_dufl(side_flag *SF,trmsrf *surf2,
trmsrf *surf3,bd_box3D *B,prat_main_blend *G,int *exc,
included_sides *inc,double eps_inc)
		...
		if(fl==-1)
			...
			if(nd!=-1)
				...
				if(ts_amo==0)
					...
					if(G->N[w].val==2)	exc[w]=+1;
					if((G->N[nd].val==3)&&(G->N[nd].type_node==T_TYPE))	exc[nd]=+1;
					if((G->N[nd].val==2)&&(G->N[nd].type_node==Q_TYPE))	exc[nd]=+1;
					e++;
					}
				}
			}
...
void lunh_trea_jidh(side_flag *SF,trmsrf *surf2,
trmsrf *surf3,bd_box3D *B,prat_main_blend *G,int *exc,
included_sides *inc,double eps_inc)
{int i,j,w,e,*sid,nd,val,fl;
int ts_amo,q,n_sid,nnd,SD;
...
{if(sid==ON_ALPHA)	nepf_disp_bulp(surf.pc.alpha);
if(sid==ON_BETA)	nepf_disp_bulp(surf.pc.beta);
if(sid==ON_GAMMA)	nepf_disp_bulp(surf.pc.gamma);
}



void interface_inc_dead(int *wr,int *sd,int n_dd,int *map,side_flag *SF,
trmsrf *surf2,trmsrf *surf3,bd_box3D *B,
prat_main_blend *G,int *exc,included_sides *inc,double eps_inc)
...
{if(sd==ON_ALPHA)	fprintf(tmpout,"ON_ALPHA");
if(sd==ON_BETA)		fprintf(tmpout,"ON_BETA");
if(sd==ON_GAMMA)	fprintf(tmpout,"ON_GAMMA");
}



void loc_ent_include(trmsrf *surf3,side_flag *SF,
hash_entry *gap,int n_gap,prat_main_blend *g_loc,prat_main_blend *G,
int *map,int *exc)
...
void det_edges_inc_simple(trmsrf *surf2,trmsrf *surf3,
bd_box3D *B,prat_main_blend *G,int *exc,included_sides *inc,
double eps_min,double eps_max,int n_trials)
{int i,ts,nnd;
...
void det_graph_bl_simple(sphere *S,trmsrf *surf2,int nb_surf2,
trmsrf *surf3,int nb_surf3,blend_cpx BC,
prat_main_blend *G,included_sides *inc)
...
void cetm_find_cojq_vurg(trmsrf *surf2,trmsrf *surf3,
bd_box3D *B,prat_main_blend *G,int *exc,included_sides *inc,
double eps_min,double eps_max,int n_trials)
{int i,ts,nnd;
double eps_inc,step,lambda;
