...
void zish_allo_gubw(trmsrf *surf1,int nb_surf1,
int nb_surf2,int nb_surf3,msh_corn *M_C)
{int i,j,k,nin,nb;
k=0;
...
void gosk_dest_togw(trmsrf *surf1,int nb_surf1,
int nb_surf2,int nb_surf3,msh_corn *M_C)
{int i,j,k,nin;
k=0;
for(i=0;i<nb_surf2;i++)
...
void quhb_allo_cutg(trmsrf *surf1,int nb_surf1,
int nb_surf2,int nb_surf3,point **MID)
{int i,j,k,nin,nb;
k=0;
for(i=0;i<nb_surf2;i++)
...
void jadc_dest_zufc(trmsrf *surf1,int nb_surf1,
int nb_surf2,int nb_surf3,point **MID)
{int i;
...
void pond_find_podj_pokt(double marg,megamanif *MG,supp_surf *sp_sr,
supp_surf *ssr,rgb_lk *col,int NED,
msh_corn *M_C,point **MID,manif_tl *msh,int *force_term)
...
int jocm_find_pijw(manif_tl msh,int el,
int *bulk,hash_entry *H,int comp_id)
{int id=-1,nel,i,p;
nel=msh.e_grs;
for(p=0;p<H[el].nb;p++)
	{i=H[el].list[p];
...
int turf_expa_lewh(manif_tl *msh,int *comp,
int comp_cur,hash_entry *H)
...
void luvm_blen_foms(atom *A,int nb_sph,int *supp,  
trmsrf *surf1,int nb_surf1,trmsrf *surf2,
int nb_surf2,trmsrf *surf3,int nb_surf3,
set_arcs *SA,blend_cpx BC,hash_entry *gap,int n_gap,
double c_param,int *forc_term)
...
sazk_disc_tujc(pd_c,pd_f,A,nb_sph,supp,surf1,nb_surf1,
surf2,nb_surf2,surf3,nb_surf3,BC,SA,&MG_c,&MG_f,sp_sr_c,sp_sr_f,
&GM_c,&GM_f,mx_edge_m,M_C_c,M_C_f,MID_c,MID_f,&f_trm);
if(f_trm==1)
	{*forc_term=1;
...
qazf_popu_fawj(msh_f,col_f,surf1,surf2,surf3,
ssr_f,&msh_fin,col,S);
...
if(verbose_variable==VERBOSE)
	...
	hojr_four_coqd(choic,ex,demten,msh_fin,col,S,
	f_max_nnd,f_max_nel,f_max_ned,&f_pv);
	if(f_pv==1)
		forc_pave=1;
	else
