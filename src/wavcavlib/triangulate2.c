...
int nojp_test_keqb(mult_conn P,
int *S,int lS,int i,int pos,int k)
...
if(nx_k==lS)
	nx_k=0;
ts_enc=kozf_test_luzk(P.vertex[S[k]],P.vertex[S[nx_k]],
P.vertex[S[pr_k]],P.vertex[i]);
if(ts_enc==0)
	return 0;
...
if(en==0)
	...
	for(j=0;j<lS;j++)
		...
		e=S[nx];
		if((p!=b)&&(p!=e)&&(i!=b)&&(i!=e))
			{ts=pumj_test_rewf(P.vertex[p],P.vertex[i],
			P.vertex[b],P.vertex[e]);
			if(ts==1)
				{ind=2;
				break;
				}
...
if(k==lengthS-1)   nx=S[0];
else			   nx=S[k+1];
if(k==0)		   pr=S[lengthS-1];
else			   pr=S[k-1];
...
int zotj_find_vemk(mult_conn P,int *S,int lS,
int f,int j,int *l,double *angle,double beta,int search_type)
...
void kupq_find_vifr(mult_conn P,int *S,int lS,
int f,int j,int *l,double *angle,double beta)
{int suc;
...
int legh_find_qijk(mult_conn P,int *remain,int lg_rem,int type,int *S,int lS,
double *ratio,double *angle,int *L,int *crv,int *vrt,double beta)
...
for(i=0;i<f_length;i++)
	{s=f[i];
	if(type==TOP)		G=zumf_find_lupk_lecn(P,s,ext);
	if(type==BOTTOM)	G=relg_bott_jaln(P,s,ext);
	if(type==RIGHT)		G=jurt_righ_qomj(P,s,ext);
	if(type==LEFT)		G=miwk_left_jitp(P,s,ext);
	for(j=0;j<G;j++)
		{refer=ext[j];
...
int tedw_find_vigs(mult_conn P,int *remain,int lg_rem,
int *S,int lS,double *RATIO,double *ANGLE,int *L,
int *CRV,int *VRT,int *type,double beta)
...
nb=legh_find_qijk(P,remain,lg_rem,BOTTOM,S,lS,
ratio,angle,l,crv,vrt,beta);
for(i=0;i<nb;i++)
	{CRV[k] =crv[i];
	RATIO[k]=ratio[i];
	ANGLE[k]=angle[i];
...
nb=legh_find_qijk(P,remain,lg_rem,RIGHT,S,lS,
ratio,angle,l,crv,vrt,beta);
for(i=0;i<nb;i++)
	{CRV[k] =crv[i];
	RATIO[k]=ratio[i];
...
nb=legh_find_qijk(P,remain,lg_rem,LEFT,S,lS,
ratio,angle,l,crv,vrt,beta);
for(i=0;i<nb;i++)
	{CRV[k] =crv[i];
	RATIO[k]=ratio[i];
...
while(1)
	{ind=1;
	lg=0.0;
	for(i=0;i<N;i++)if(alpha[i]>theta)
		{if(ratio[i]>lg)
			{lg=ratio[i];
			res=i;
			ind=2;
...
void kudm_gene_ramv(mult_conn P,int *S,
int *lengthS,double beta,parm *pass,int *lp)
...
void triangulate_2D_simple(mult_conn P,manif_ro *msh,
double anisotropy,double div,int max,int *forc_term)
{int suc,f_trm;
...
void feht_tria_duvs(polygon Q,int max,
manif_ro *msh,int *forc_term)
