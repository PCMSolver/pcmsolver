...
void degj_para_mojc(trm_sph S,
double u,double v,point *P)
{double den;
den=1.0+u*u+v*v;
...
void peht_find_ligd_mebn(vect3D W_in,
double phi,double theta,vect3D *W_out)
{int i;
double **A,*V_in,*V_out;
vect3D temp;
...
void tulr_find_rads_tojm(vect3D S_in,
double phi,double theta,vect3D *S_out)
{int i;
double **A,*V_in,*V_out;
vect3D tp;
V_in=(double *)malloc(3*sizeof(double));
