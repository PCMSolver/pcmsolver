...
void ponv_stat_jozq(c_arc2D C,int *state_bt,
int *state_tp,double *al_bt,double *al_tp)
{int i;
...
void zacd_spli_tugd(c_arc2D C,double *al,
int nb,c_arc2D *c)
{int i;
parm *sep;
if(nb==0)
...
par=nb % 2;
if(par==1)	res=INS_SET;
if(par==0)	res=OUT_SET;
return res;
}
...
free(p);
BX->x_min=xmi;	BX->x_max=xma;
BX->y_min=ymi;	BX->y_max=yma;
}
...
for(i=0;i<A.ar_grs;i++)
	...
	if(bx.x_min<xmi)	xmi=bx.x_min;
	if(bx.x_max>xma)	xma=bx.x_max;
	if(bx.y_min<ymi)	ymi=bx.y_min;
	if(bx.y_max>yma)	yma=bx.y_max;
	}
BX->x_min=xmi;	BX->x_max=xma;
BX->y_min=ymi;	BX->y_max=yma;
}
...
int mosc_encl_hapk(polyarc *P,int N,int k,int *excl,
int m,int *list)
...
void pomq_curr_jecf(polygon *P,polyarc *P_arc,int n,
int *excl,int m,int *EX,int *IN,int *NIN)
{int ex,nin,*list,i;
...
int qonk_form_sevt(int par,trm_sph T,int nb_arcs,
c_curve *cc,int n,trmsrf *surf,int nb_cur,
int *supp,int max_surf)
