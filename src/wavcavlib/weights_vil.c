...
double wotf_stat_kefz(double lambda,point A1,point B1,
point C1,point A2,point B2,point C2,int *idx)
{double res;
parm a1,b1,c1,b2;
vect3D N1,N2,N,U,V;
...
void tukc_comm_nudj(manif_tl msh,int r,int s,
int *a,int *b,int *forc_term)
...
double rocq_stat_todr(double lambda,manif_tl msh,
int r,int s,int *idx,int *forc_term)
...
res=wotf_stat_kefz(lambda,msh.knot[a1],msh.knot[b1],
msh.knot[c1],msh.knot[a2],msh.knot[b2],msh.knot[c2],idx);
return res;
}
