...
unsigned int wavelet_wavelet_criterion(bounding_box_gamma *B1, bounding_box_square *B2, wavelet *W, element *E, unsigned int ind1, unsigned int ind2, double c1, double c2);
...
unsigned int M;                 /* Zahl der Level             */
{
    ...
    for (i = 0; i < nf; i++) {
        ...
        for (j = 1; j < W[i].element_number; j++) {
            ...
            if (E[k].index_s < min_x * (1 << E[k].level))
                min_x = E[k].index_s * 1. / (1 << E[k].level);
            if (E[k].index_t < min_y * (1 << E[k].level))
                min_y = E[k].index_t * 1. / (1 << E[k].level);
            if (E[k].index_s + 1 > max_x * (1 << E[k].level))
                max_x = (E[k].index_s + 1) * 1. / (1 << E[k].level);
            if (E[k].index_t + 1 > max_y * (1 << E[k].level))
                max_y = (E[k].index_t + 1) * 1. / (1 << E[k].level);
        }
        (*B)[i].r_x = 0.5 * (max_x - min_x);
        (*B)[i].r_y = 0.5 * (max_y - min_y);
...
/* 2. Kompression: beide Wavelets leben auf dem selben Patch */
    if (B2[ind1].patch == B2[ind2].patch) {
        ...
        if (dx + dy > 0) {      /* Wavelets sind disjunkt */
            if (dx * dx + dy * dy >= c2 * c2)
                return (0);
            else
                return (1);
            for (i = 0; i < W[ind1].element_number; i++) {
                for (j = 0; j < W[ind2].element_number; j++) {
                    if (distance(&E[W[ind1].element[i]], &E[W[ind2].element[j]]) < c2)
                         return (1);
                }
            }
        } else {                /* Wavelets sind nicht disjunkt *//* teste, ob W[ind1]+c2 \not\subset W[ind2] */
            if ((B2[ind1].m_x - B2[ind1].r_x - c2 < B2[ind2].m_x - B2[ind2].r_x)
                || (B2[ind1].m_x + B2[ind1].r_x + c2 > B2[ind2].m_x + B2[ind2].r_x) || (B2[ind1].m_y - B2[ind1].r_y - c2 < B2[ind2].m_y - B2[ind2].r_y) || (B2[ind1].m_y + B2[ind1].r_y + c2 > B2[ind2].m_y + B2[ind2].r_y))
                return (1);
            ...
            for (j = 0; j < W[ind2].element_number; j++) {
                ...
                if (dx + dy > 0) {      /* Wavelet und Element sind disjunkt */
                    ...
                    if ((B2[ind1].m_x - B2[ind1].r_x - c2 < s2 - h2) || (B2[ind1].m_x + B2[ind1].r_x + c2 > s2 + h2) || (B2[ind1].m_y - B2[ind1].r_y - c2 < t2 - h2) || (B2[ind1].m_y + B2[ind1].r_y + c2 > t2 + h2))
                        return (1);
                }
            }
        }
