    ...
    for (m = M; m >= minLevel; m--) {
        ...
        for (i1 = 0; i1 < p; i1++) {
            ...
            for (i2 = 0; i2 < n; i2++) {
                for (i3 = 0; i3 < n; i3++) {
                    /* Skalierungsfunktion */
                    b[zg + n * i2 + i3] = 0.5 * (a[4 * zg + (4 * n * i2) + (2 * i3)] + a[4 * zg + (4 * n * i2) + (2 * i3 + 1)] + a[4 * zg + (4 * n * i2 + 2 * n) + (2 * i3)] + a[4 * zg + (4 * n * i2 + 2 * n) + (2 * i3 + 1)]);

                    /* Wavelet: psi(s)*phi(t) */
                    for (s = 0; s < T.row_number[i3]; s++) {
                        b[zg + p * n * n + n * i2 + i3] += 0.5 * T.value[i3][s] * (a[4 * zg + (4 * n * i2) + T.index[i3][s]] + a[4 * zg + (4 * n * i2 + 2 * n) + T.index[i3][s]]);
                    }
