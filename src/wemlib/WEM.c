...
{  
   ...
   {  if (S->index[E[i].wavelet[j]][0] < test_wavelet) test_wavelet = S->index[E[i].wavelet[j]][0];
   ...
   while ((test_wavelet < S->n) && (E[i].level+2 > W[test_wavelet].level))
   {
      ...
      {  
         ...
         {  element_element_interaction(c,E,i,W[test_wavelet].element[j],RW,Q,T,M,\
      ...
      test_wavelet = S->n;
      for (j=0; j<E[i].wavelet_number; j++)
      {  if (S->index[E[i].wavelet[j]][ansatz_wavelet[j]] < test_wavelet) test_wavelet = S->index[E[i].wavelet[j]][ansatz_wavelet[j]];
         }
      }
