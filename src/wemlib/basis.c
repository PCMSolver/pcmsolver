...
/* 1. vereinfache die Wavelets */
	for (i = 0; i < nw; i++) {
		if (W[i].level > minLevel) {	/* Untersuche, ob Feingitterelemente durch ihren Vater ersetzt werden   */
			...
			for (s0 = 0; s0 < noe; s0++) {
				...
				if (E[ind].level == W[i].level) {	/* also Feingitterelement -> suche nach den Soehnen *//* ueberpruefe, ob zu untersuchendes Element der 0. Sohn ist */
					ind = E[ind].father;	/* Vaterelement */
					if (W[i].element[s0] == E[ind].son[0]) {	/* 0. Sohn vorhanden -> ueberpruefe auf 1. Sohn */
						for (s1 = 0; (s1 < noe) && (E[ind].son[1] != W[i].element[s1]); s1++);
						if ((s1 < noe) && (W[i].weight[s1] == W[i].weight[s0])) {	/* 0.+1. Sohn vorhanden -> ueberpruefe auf 2. Sohn */
							for (s2 = 0; (s2 < noe) && (E[ind].son[2] != W[i].element[s2]); s2++);
							if ((s2 < noe) && (W[i].weight[s2] == W[i].weight[s0])) {	/* 0.+1.+2. Sohn vorhanden -> ueberpruefe auf 3. Sohn */
								for (s3 = 0; (s3 < noe) && (E[ind].son[3] != W[i].element[s3]); s3++);
								if ((s3 < noe) && (W[i].weight[s3] == W[i].weight[s0])) {	/* alle 4 Soehne sind vorhanden und haben gleiche Gewichte */
									W[i].element    [s0] = ind;
									W[i].weight[s0] *= 2;
