...
if (CASE < 3) WEM(&S_i,W,E,T,p,M,SingleLayerInt,DoubleLayerInt,2*pi*(1+epsilon)/(1-epsilon));
else          WEM(&S_i,W,E,T,p,M,SingleLayerInt,DoubleLayerInt,2*pi);
...
{  time(&t2);
   ...
   if (CASE == 3) WEM(&S_e,W,E,T,p,M,SingleLayerExt,DoubleLayerExt,-2*pi);
   else           WEM(&S_e,W,E,T,p,M,SingleLayerAni,DoubleLayerAni,-2*pi);
...
{  WEMRHS2(&rhs,W,E,T,p,M);
   ...
   tdwtKon(u,M,nf);
   dwtKon(u,M,nf);
   for (i=0; i<nf; i++) rhs[i] += 4*pi*u[i]/(epsilon-1);
...
{  WEMRHS2(&rhs,W,E,T,p,M);
   ...
   printf("Solving the 2nd linear system:   %d iterations\n",i);   
   for (i=0; i<nf; i++) u[i] -= 4*pi*v[i]; 	/* u = u - 4*pi*v */ 
   }
time(&t2);
printf("Computation time:                %g secs.\n\n",difftime(t2,t3));
