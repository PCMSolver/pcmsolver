...
while (low < high)
{  mid = (low+high)/2;
   if (array[mid] < j)        low = mid+1;
   else if (array[mid] > j)   high = mid;
   else return(mid);
   }
return(low);		/* low == high! */
}
...
{  /* Anzahl Elemente in Zeile i muss erhoeht werden */
   n_max = rn + 10;
...
for (i=0; i<m; i++)
{  /* 1 Dummy-Element: Waechter- + Pufferelement */
   A->max_row_number[i] = n_max;
...
k = search_sparse(A->index[i],A->row_number[i],j);
if (A->index[i][k] == j) A->value[i][k] += z;	/* Eintrag schon vorhanden      */
...
k = search_sparse(A->index[i],A->row_number[i],j);
if (A->index[i][k] == j) return(A->value[i][k]);	/* Eintrag vorhanden       */
else return(0);  		  			/* Eintrag nicht vorhanden */   
}
