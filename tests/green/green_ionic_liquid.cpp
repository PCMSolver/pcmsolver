...
TEST_CASE("Evaluation of the ionic liquid Green's function and its derivatives", "[green][green_ionic_liquid]")
{
    double epsilon = 60.0;
    double kappa = 5.0;
    ...
    Eigen::Array4d result = analyticIonicLiquid(epsilon, kappa, sourceNormal, source, probeNormal, probe);
    ...
    {
        IonicLiquid<Numerical, CollocationIntegrator> gf(epsilon, kappa);
    ...
    {
        IonicLiquid<AD_gradient, CollocationIntegrator> gf(epsilon, kappa);
        double value = result(0);
        double gf_value = gf.kernelS(source, probe);
        REQUIRE(value == Approx(gf_value));
    ...
    {
        IonicLiquid<AD_hessian, CollocationIntegrator> gf(epsilon, kappa);
        double value = result(0);
        double gf_value = gf.kernelS(source, probe);
        REQUIRE(value == Approx(gf_value));
